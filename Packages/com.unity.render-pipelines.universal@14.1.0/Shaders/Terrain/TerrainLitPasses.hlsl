...
Varyings SplatmapVert(Attributes v)
{
    ...
    o.dynamicLightmapUV = v.texcoord * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
        ...
        // For Terrain, there's one situation that Mesh Renderer uses "TerrainLit.shader", which will
        // result in that VG cannot get the right value by SampleSH. Under this situation, base color,
        // aka albedo, is zero, e.g. black, so SH value will not take effect for rendering's result.

...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    internal class DeferredLights
    {
        ...
        private static readonly string[] k_DeferredPlusPassNames = {
            "Deferred Plus Lights (Lit)",
            "Deferred Plus Lights (SimpleLit)",
        };
        ...
        private enum DeferredPlusPasses
        {
            Lit,
            SimpleLit,
        };
        ...
        static readonly string k_DeferredPlusShadingPass = "Deferred Shading (Deferred+)";
        ...
        private Material m_DeferredPlusMaterial;

        // Pass indices.
        int[] m_StencilDeferredPasses;
        private int[] m_DeferredPlusPasses;
        ...
        ProfilingSampler m_ProfilingSamplerDeferredPlusShadingPass = new ProfilingSampler(k_DeferredPlusShadingPass);
        ...
        internal struct InitParams
        {
            ...
            public Material deferredPlusMaterial;
        ...
        internal DeferredLights(InitParams initParams, bool useNativeRenderPass = false)
        {
            ...
            // Cache result for DX10 platform too. Same reasons as above.
            ...
            m_DeferredPlusMaterial = initParams.deferredPlusMaterial;
            
            m_StencilDeferredPasses = new int[k_StencilDeferredPassNames.Length];
            m_DeferredPlusPasses = new int[k_DeferredPlusPassNames.Length];
            ...
            m_DeferredPlus = initParams.deferredPlus;
        }
        ...
        internal void SetMixedLightingMode(MixedLightingSetup setup)
        {
            MixedLightingSetup = setup;
        ...
        internal void ResolveMixedLightingMode(ref RenderingData renderingData)
        {
...
#endif
            {
                ...
                {
                    // seems like light is not modified, so we can use ReadOnly version?
        ...
        internal void ExecuteDeferredPass(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (m_DeferredPlusPasses[0] < 0) InitDeferredPlusShadingMaterial();    
            if (m_StencilDeferredPasses[0] < 0) InitStencilDeferredMaterial();    
            ...
            using (new ProfilingScope(cmd, m_ProfilingDeferredPass))
            {
                ...
                {
                    // Always apply SSAO in deferred+ full screen shading pass
                    RenderDeferredPlusLights(cmd);
                }
        ...
        {
            cmd.SetGlobalMatrixArray(ShaderConstants._ScreenToWorld, GetScreenToWorldMatrix(renderingData.cameraData));
        }
        ...
        void RenderDeferredPlusLights(CommandBuffer cmd)
        {
            Profiler.BeginSample(k_DeferredPlusShadingPass);
            ...
            using (new ProfilingScope(cmd, m_ProfilingSamplerDeferredPlusShadingPass))
            {
                cmd.DrawMesh(m_FullscreenMesh, Matrix4x4.identity, m_DeferredPlusMaterial, 0, m_DeferredPlusPasses[(int)DeferredPlusPasses.Lit]);
                cmd.DrawMesh(m_FullscreenMesh, Matrix4x4.identity, m_DeferredPlusMaterial, 0, m_DeferredPlusPasses[(int)DeferredPlusPasses.SimpleLit]);
            }
        ...
        {
            if (m_DeferredPlusMaterial == null)
                return;
            
            // Pass indices can not be hardcoded because some platforms will strip out some passes, offset the index of later passes.
            for (int pass = 0; pass < k_DeferredPlusPassNames.Length; ++pass)
                m_DeferredPlusPasses[pass] = m_DeferredPlusMaterial.FindPass(k_DeferredPlusPassNames[pass]);
            
            m_DeferredPlusMaterial.SetFloat(ShaderConstants._LitStencilRef, (float)StencilUsage.MaterialLit);
            m_DeferredPlusMaterial.SetFloat(ShaderConstants._LitStencilReadMask, (float)StencilUsage.MaterialMask);
            m_DeferredPlusMaterial.SetFloat(ShaderConstants._SimpleLitStencilRef, (float)StencilUsage.MaterialSimpleLit);
            m_DeferredPlusMaterial.SetFloat(ShaderConstants._SimpleLitStencilReadMask, (float)StencilUsage.MaterialMask);
        }

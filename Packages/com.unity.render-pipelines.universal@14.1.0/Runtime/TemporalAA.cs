...
namespace UnityEngine.Rendering.Universal
{
    ...
    sealed internal class TaaPersistentData : ITemporalPersistentHistory
    {
    ...
    public static class TemporalAA
    {
        ...
        static internal float[] CalculateFilterWeights(float jitterScale)
        {
            ...
            for (int i = 0; i < 9; ++i)
            {
                Vector2 jitter = TemporalUtils.CalculateJitter(Time.frameCount) * jitterScale;
        ...
        internal static void ExecutePass(CommandBuffer cmd, Material taaMaterial, ref CameraData cameraData, RTHandle source, RTHandle destination, RenderTexture motionVectors)
        {
            ...
            {
                TaaPersistentData taaPersistentData = cameraData.temporalPersistentData as TaaPersistentData;
                Debug.Assert(taaPersistentData != null);
                ...
                bool isNewFrame = taaPersistentData.GetLastAccumFrameIndex(multipassId) != Time.frameCount;

                RTHandle taaHistoryAccumulationTex = taaPersistentData.accumulationTexture(multipassId);
                ...
                CoreUtils.SetKeyword(taaMaterial, ShaderKeywordStrings._PRESERVE_ALPHA, cameraData.preserveAlpha);
                ...
                if (isNewFrame)
                {
                    ...
                    taaPersistentData.SetLastAccumFrameIndex(multipassId, Time.frameCount);
                }
            }
        }
        ...
        internal static void Render(RenderGraph renderGraph, Material taaMaterial, ref CameraData cameraData, ref TextureHandle srcColor, ref TextureHandle srcDepth, ref TextureHandle srcMotionVectors, ref TextureHandle dstColor)
        {
            ...
            RTHandle accumulationTexture = taaPersistentData.accumulationTexture(multipassId);
            ...
            using (var builder = renderGraph.AddRenderPass<TaaPassData>("Temporal Anti-aliasing", out var passData, ProfilingSampler.Get(URPProfileId.TemporalAA)))
            {
                ...
                passData.preserveAlpha = cameraData.preserveAlpha;
                ...
                builder.SetRenderFunc((TaaPassData data, RenderGraphContext context) =>
                {
                    ...
                    CoreUtils.SetKeyword(data.material, ShaderKeywordStrings._PRESERVE_ALPHA, data.preserveAlpha);

...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// <summary>
    /// This controls the size of the Reflection Probe Atlas, which used for VG instances.
    /// </summary>
    public enum ReflectionProbeAtlasResolution
    {
        /// <summary>
        ...
        /// </summary>
        _1024x512 = 1024 << 16 | 512,

        /// <summary>
        /// Resolution 1024 x 1024
        /// </summary>
        _1024x1024 = 1024 << 16 | 1024,

        /// <summary>
        /// Resolution 2048 x 1024
        /// </summary>
        _2048x1024 = 2048 << 16 | 1024,

        /// <summary>
        /// Resolution 2048 x 2048
        /// </summary>
        _2048x2048 = 2048 << 16 | 2048,

        /// <summary>
        /// Resolution 4096 x 2048
        /// </summary>
        _4096x2048 = 4096 << 16 | 2048,

        /// <summary>
        /// Resolution 4096 x 4096
        /// </summary>
        _4096x4096 = 4096 << 16 | 4096,

        /// <summary>
        /// Resolution 8192 x 4096
        /// </summary>
        _8192x4096 = 8192 << 16 | 4096,

        /// <summary>
        /// Resolution 8192 x 8192
        /// </summary>
        _8192x8192 = 8192 << 16 | 8192
    }
    ...
    public enum UpscalingFilterSelection
    {
        ...
        /// <summary>
        /// Unity uses the Tuanjie Super Resolution technique to perform upscaling.
        /// </summary>
        [InspectorName("Tuanjie Super Resolution (TJSR)"), Tooltip("A temporal Post-Processing Upscaler providing a good balance between performance and visual quality mainly for Mobile and VR/XR applications.")]
        TJSR
    }

    public enum AdvancedUpscalingFilterOption
    {
        /// <summary>
        /// None means there is no available AdvancedUpscalingFilter on current platform.
        /// This option should not be shown in Inspector.
        /// </summary>
        None = 0,
        
        /// <summary>
        /// MetalFX is available on macOS or iOS.
        /// </summary>
        [InspectorName("MetalFX Upscaling (MetalFX)"), Tooltip("Only works on iOS and MacOS.")]
        MetalFX,
    }

    public enum MetalFXModeSelection
    {
        [InspectorName("Spatial Upscaler")]
        Spatial,
        
        [InspectorName("Temporal Upscaler")]
        Temporal,
    }

    [Serializable]
    public struct TJSRSettings
    {
        /// <summary>
        /// The quality level of Tuanjie Super Resolution.
        /// </summary>
        [SerializeField]
        public TJSRQuality tjsrQuality;

        /// <summary>
        /// Base Blend Factor of Tuanjie Super Resolution. The default value is zero.
        /// To blend history color value with the accumulate value if this blend factor is 0.
        /// If the camera view is discontinuous or the scene is switching, the accumulation should be reset, 
        /// then set this blend factor to greater than one to force to use accumulate color value from the current frame.
        /// </summary>
        [SerializeField]
        [Range(0,1)]
        public float tjsrBaseBlendFactor;

        /// <summary>
        /// Contrast Adaptive Sharpening for Tuanjie Super Resolution. Only enabled in High Quality.
        /// </summary>
        [SerializeField] [Range(0, 1)]
        private float m_tjsrContrastAdaptiveSharpening;
        
        public float tjsrContrastAdaptiveSharpening => tjsrQuality == TJSRQuality.High ? m_tjsrContrastAdaptiveSharpening : 0.0f;

        public TJSRSettings(TJSRQuality quality, float baseBlendFactor, float contrastAdaptiveSharpening)
        {
            tjsrQuality = quality;
            tjsrBaseBlendFactor = baseBlendFactor;
            m_tjsrContrastAdaptiveSharpening = contrastAdaptiveSharpening;
        }
    }

    public enum TJSRQuality
    {
        [Tooltip("Low quality, use 2-pass fragment shaders, favors speed and low power consumption, and optimal for low-end Mobile devices.")]
        Low,
        [Tooltip("Medium quality, use 2-pass fragment shaders, a good balance between performance and visual quality, and recommended for Mobile and VR/XR applications.")]
        Medium,
        [Tooltip("High quality, use 3-pass fragment shaders and more GPU bandwidth, and often optimal for Desktop/Console and higher-end Mobile applications.")]
        High
    }
    ...
    public partial class UniversalRenderPipelineAsset : RenderPipelineAsset, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] AdvancedUpscalingFilterOption[] m_AdvancedUpscalingFilterList = new AdvancedUpscalingFilterOption[]{ };
        [SerializeField] TJSRSettings m_TJSRSettings = new TJSRSettings(TJSRQuality.Medium, 0.0f/*baseBlendFactor*/, 0.0f/*contrastAdaptiveSharpening*/);
        [SerializeField] MetalFXModeSelection m_MetalFXMode = MetalFXModeSelection.Spatial;
        ...
        [SerializeField] ReflectionProbeAtlasResolution m_ReflectionProbeAtlasResolution = ReflectionProbeAtlasResolution._2048x2048;
...
#if UNITY_EDITOR
        [ShaderKeywordFilter.SelectOrRemove(true, keywordNames: ShaderKeywordStrings._PRESERVE_ALPHA)]
#endif
        [SerializeField] bool m_PostProcessPreserveAlpha = false;

        // GPU Resident Drawer
        [SerializeField] GPUResidentDrawerStat m_GPUResidentDrawerStat = GPUResidentDrawerStat.Disabled;
        [SerializeField] bool m_GRDEnableCollectTransform = true;
        [SerializeField] bool m_GRDEnableCollectLightMap = false;
        [SerializeField] bool m_GRDEnableCollectLightProbe = false;
        [SerializeField] bool m_GRDEnableCollectMotionVector = false;
        [SerializeField] GRDDataStoreType m_GRDUseTextureStoreData = GRDDataStoreType.UseTexture;

        [SerializeField] BRGShaderVariantStorageStripType m_BRGShaderVariantStorageStripType = BRGShaderVariantStorageStripType.KeepSSBOOrUniform;
...
#if UNITY_EDITOR
        /// <summary>
        ///  This is used to force enable GRD, should be used for test case. If asset not support, enable this can force grd enable and may get wrong rendering.s
        /// </summary>
        public bool testURPForceGRD = false;
#endif 
        ...
        public bool IsGrdAvailable()
        {
            ...
            return
                ...
                testURPForceGRD || 
#endif
                (IsGRDRenderingModeSupport(this)
#if UNITY_EDITOR
                && EditorGraphicsSettings.batchRendererGroupStrippingMode == BrgStrippingMode.KeepAll
                && ((brgShaderVariantStorageStripType != BRGShaderVariantStorageStripType.KeepSSBOOrUniform && grdUseTextureStoreData == GRDDataStoreType.UseTexture) ||
                    (brgShaderVariantStorageStripType != BRGShaderVariantStorageStripType.KeepTexture && grdUseTextureStoreData == GRDDataStoreType.UseSSBO)
                    )
#endif
                )
                ;
        ...
        public void SetupGRDSettings()
        {
            GPUScene.enableGRD = gpuResidentDrawerStat == GPUResidentDrawerStat.InstancedDrawing && IsGrdAvailable() ? true : false;
            if (GPUScene.enableGRD)
            {
                GRDInstancePropertyFlag globalGRDInstancePropertyflag = GRDInstancePropertyFlag.InvalidProperty;
                globalGRDInstancePropertyflag |= GRDInstancePropertyFlag.GRDObjectToWorld | GRDInstancePropertyFlag.GRDWorldToObject;

                globalGRDInstancePropertyflag |= grdEnableCollectLightMap ? GRDInstancePropertyFlag.GRDLightmapST : GRDInstancePropertyFlag.InvalidProperty;
                globalGRDInstancePropertyflag |= grdEnableCollectLightProbe ? GRDInstancePropertyFlag.GRDLightProbe : GRDInstancePropertyFlag.InvalidProperty;

                if (globalGRDInstancePropertyflag != GPUScene.globalGRDInstancePropertyflag)
                    GPUScene.globalGRDInstancePropertyflag = globalGRDInstancePropertyflag;
#if PLATFORM_WEIXINMINIGAME || PLATFORM_WEBGL
                GPUScene.SetUseTexture(true);
#else 
                GPUScene.SetUseTexture(grdUseTextureStoreData == GRDDataStoreType.UseTexture);
#endif
            }
        }
...
#if UNITY_GPU_DRIVEN_PIPELINE
            // enable GDRP not use grd
            if (!GraphicsSettings.enableGDRP)
#endif
            {
                GRDFilterSetting setting = new GRDFilterSetting();
                setting.maxRenderQueue = 2500;
                setting.settingflag = 1;
                setting.opType = GRDFilterOpType.Or;

                GPUScene.AddGRDFilterSetting(new GRDFilterSetting[] { setting });
            }
        ...
        protected override void OnDisable()
        {
            ...
            if (!GraphicsSettings.enableGDRP)
#endif
                GPUScene.ClearGRDFilterSetting();
        }
        ...
        /// <summary>
        /// Preserves alpha through post-processing.
        /// </summary>
        public bool postProcessPreserveAlpha
        {
            get { return m_PostProcessPreserveAlpha; }
            set { m_PostProcessPreserveAlpha = value; }
        }
        ...
        /// <summary>
        /// Advanced Upscaling filter list.
        /// </summary>
        public AdvancedUpscalingFilterOption[] advancedUpscalingFilterList
        {
            get { return m_AdvancedUpscalingFilterList; }
            set { m_AdvancedUpscalingFilterList = value; }
        }

        /// <summary>
        /// Return the supported AdvancedUpscalingFilterOption on current platform.
        /// </summary>
        /// <returns>
        /// None if the user does not specify an advanced upscaling filter option,
        /// or all specified options are not supported on current platform.
        /// </returns>
        public AdvancedUpscalingFilterOption ResolveAdvancedUpscalingFilterOption()
        {
            foreach (var filter in m_AdvancedUpscalingFilterList)
            {
                switch (filter)
                {
                    case AdvancedUpscalingFilterOption.MetalFX:
#if UNITY_STANDALONE_OSX || UNITY_EDITOR_OSX || UNITY_IOS
                        if (MetalFXCoreUtils.IsSupported())
                        {
                            return AdvancedUpscalingFilterOption.MetalFX;
                        }
                        else
                        {
                            const int warningThrottleFrames = 60;
                            if (Time.frameCount % warningThrottleFrames == 0)
                                Debug.LogWarning("MetalFX is not enabled because it is not supported on this device.");
                        }
#endif
                        break;
                    case AdvancedUpscalingFilterOption.None:
                    default:
                        break;
                }
            }

            return AdvancedUpscalingFilterOption.None;
        }

        /// <summary>
        /// Tuanjie Super Resolution settings.
        /// </summary>
        public TJSRSettings tjsrSettings
        {
            get { return m_TJSRSettings; }
            set { m_TJSRSettings = value; }
        }

        /// <summary>
        /// MetalFX upscaling Mode.
        /// </summary>
        public MetalFXModeSelection metalFXMode
        {
            get { return m_MetalFXMode; }
            set { m_MetalFXMode = value; }
        }
        ...
        internal Vector2Int reflectionProbeAtlasResolution => new Vector2Int((int)m_ReflectionProbeAtlasResolution >> 16, (int)m_ReflectionProbeAtlasResolution & 0xFFFF);
...
#pragma warning restore 618 // Obsolete warning
        ...
        public static bool IsURPAssetSupportGDRP()
        {
#if UNITY_GPU_DRIVEN_PIPELINE
            var urpAsset = GraphicsSettings.currentRenderPipeline as UniversalRenderPipelineAsset;
            return (urpAsset?.scriptableRenderer as UniversalRenderer)?.renderingModeActual == RenderingMode.Deferred;
#else
            return false;
#endif
        }

        public enum GPUResidentDrawerStat : byte
        {
            Disabled,
            InstancedDrawing
        }

        public enum BRGShaderVariantStorageStripType : byte
        {
            KeepSSBOOrUniform = 0,
            KeepTexture = 1,
            KeepAll = 2
        ...
        public enum GRDDataStoreType : byte
        {
            UseTexture,
            UseSSBO
        }

        public GPUResidentDrawerStat gpuResidentDrawerStat
        {
            get => m_GPUResidentDrawerStat;
            set
            {
                if (value == m_GPUResidentDrawerStat)
                    return;

                m_GPUResidentDrawerStat = value;
                OnValidate();
            }
        }

        public bool grdEnableCollectTransform
        {
            get => m_GRDEnableCollectTransform;
            set
            {
                if (value == m_GRDEnableCollectTransform)
                    return;

                m_GRDEnableCollectTransform = value;
                OnValidate();
            }
        }

        public bool grdEnableCollectLightMap
        {
            get => m_GRDEnableCollectLightMap;
            set
            {
                if (value == m_GRDEnableCollectLightMap)
                    return;

                m_GRDEnableCollectLightMap = value;
                OnValidate();
            }
        }

        public bool grdEnableCollectLightProbe
        {
            get => m_GRDEnableCollectLightProbe;
            set
            {
                if (value == m_GRDEnableCollectLightProbe)
                    return;

                m_GRDEnableCollectLightProbe = value;
                OnValidate();
            }
        }

        public bool grdEnableCollectMotionVector
        {
            get => m_GRDEnableCollectMotionVector;
            set
            {
                m_GRDEnableCollectMotionVector = value;
                OnValidate();
            }
        }

        public GRDDataStoreType grdUseTextureStoreData
        {
            get => m_GRDUseTextureStoreData;
            set
            {
                m_GRDUseTextureStoreData = value;
                OnValidate();
            }
        }

        public BRGShaderVariantStorageStripType brgShaderVariantStorageStripType
        {
            get => m_BRGShaderVariantStorageStripType;
            set
            {
                if (value == m_BRGShaderVariantStorageStripType)
                    return;

                m_BRGShaderVariantStorageStripType = value;
            }
        }

        public static bool IsGRDRenderingModeSupport(UniversalRenderPipelineAsset asset)
        {
            foreach (var rendererData in asset.m_RendererDataList)
            {
                if (rendererData is not UniversalRendererData)
                {
                    Debug.LogWarning("GPUResidentDrawer Disabled due to some configured Universal Renderers not being UniversalRendererData.");
                    return false;
                }

                var universalRendererData = rendererData as UniversalRendererData;
                if (universalRendererData.renderingMode != RenderingMode.ForwardPlus && universalRendererData.renderingMode != RenderingMode.Forward)
                    return false;
            }
            return true;
        }

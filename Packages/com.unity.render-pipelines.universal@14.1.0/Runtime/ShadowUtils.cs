...
namespace UnityEngine.Rendering.Universal
{
    ...
    public static class ShadowUtils
    {
        ...
        /// <summary>
        /// Allocate a Main Light Shadow Hiz Map or re-allocate if it doesn't match requirements.
        /// For use only if the map requirements changes at runtime.
        /// </summary>
        /// <param name="handle">RTHandle to check (can be null).</param>
        /// <param name="width">Width of the Main Light Shadow Hiz Map.</param>
        /// <param name="height">Height of the Main Light Shadow Hiz Map.</param>
        /// <param name="bits">Minimum depth bits of the Shadow Map.</param>
        /// <param name="name">Name of the Main Light Shadow Hiz Map.</param>
        /// <returns>If the RTHandle was re-allocated</returns>
        public static bool MainLightShadowHizRTReAllocateIfNeeded(ref RTHandle handle, int width, int height, int bits, string name)
        {
            var format = GraphicsFormatUtility.GetDepthStencilFormat(bits, 0);
            // Convert from depth format to color format.
            switch (format)
            {
                case GraphicsFormat.D32_SFloat or GraphicsFormat.D24_UNorm:
                    format = GraphicsFormat.R32_SFloat;
                    break;
                case GraphicsFormat.D16_UNorm or GraphicsFormat.None:
            ...
            return RenderingUtils.ReAllocateIfNeeded(ref handle, desc, name: name);
        }

        // @TODO: We may realize this function within native code to keep consistent.
        // Attention: There's tiny difference of Mathf.Round() and std::roundf()!
        // C++ std::roundf() rounds half away from zero - https://cplusplus.com/reference/cmath/round/
        // C# Mathf.Round() rounds half to even - https://docs.unity3d.com/ScriptReference/Mathf.Round.html
        public static void CalcResizedRes(int width, int height, int targetWidth, int targetHeight, ref int newWidth, ref int newHeight, ref int ratio)
        {
            float ratioW = (float)width / (float)targetWidth; // width and height are maximum thumbnail's bounds
            float ratioH = (float)height / (float)targetHeight;
            if (ratioW > ratioH)
            {
                newWidth = targetWidth;
                newHeight = (int)(Mathf.Round((float)height / ratioW));
                ratio = (int)ratioW;
                if (width % targetWidth > 0) ratio++;
            }
            else
            {
                newWidth = (int)(Mathf.Round((float)width / ratioH));
                newHeight = targetHeight;
                ratio = (int)ratioH;
                if (height % targetHeight > 0) ratio++;
            }
        }
#endif

...
namespace UnityEngine.Rendering.Universal
{
    ...
    public enum RenderingMode
    {
        ...
        Deferred = 1,
        /// <summary>Render all objects first in a g-buffer pass, then run a full-screen pass to apply all lighting using a clustered data structure.</summary>
    ...
    public sealed partial class UniversalRenderer : ScriptableRenderer
    {
        ...
        readonly GraphicsFormat k_DepthStencilFormat;
        readonly int k_DepthBufferBits;
        ...
        internal RenderingMode renderingModeActual => renderingModeRequested is RenderingMode.Deferred or RenderingMode.DeferredPlus && (GL.wireframe || (DebugHandler != null && DebugHandler.IsActiveModeUnsupportedForDeferred) || m_DeferredLights == null || !m_DeferredLights.IsRuntimeSupportedThisFrame() || m_DeferredLights.IsOverlay)
        ? RenderingMode.Forward
        : this.renderingModeRequested;

        internal bool isDeferredLighting => renderingModeActual == RenderingMode.Deferred || renderingModeRequested == RenderingMode.DeferredPlus;
        bool isClusteredLighting => renderingModeActual == RenderingMode.ForwardPlus || renderingModeRequested == RenderingMode.DeferredPlus;
        ...
        GPUDrivenPass m_GPUDrivenPass;
        GPUReflectionProbePass m_GPUReflectionProbePass;
        GPUMotionVectorPass m_GPUMotionVectorPass;
        DecalBufferEmitPass m_DecalBufferEmitPass;
        StencilSplatPass m_StencilSplatPass;
        GBufferSplatPass m_GBufferSplatPass;
        CopyDepthPyramidPass m_CopyDepthPyramidPass;
#endif
        CopyDepthPass m_PrimedDepthCopyPass;
        MotionVectorRenderPass m_MotionVectorPass;
#if UNITY_EDITOR
        ShadingRateOnlyPass m_ShadingRateOnlyPass;
#endif
        ...
        private bool m_IsColorBufferUpscale;
        private RenderTargetBufferSystem m_ColorBufferSystem;
        private RenderTargetBufferSystem m_ColorBufferUpscaleSystem;
        internal RenderTargetBufferSystem colorBufferSystem
        {
            get => m_IsColorBufferUpscale ? m_ColorBufferUpscaleSystem : m_ColorBufferSystem;
        }
        ...
        RTHandle m_ShadingRateColor;
        RTHandle m_ShadingRateDepth;
        ...
        private Material m_DeferredPlusMaterial = null;
        Material m_CameraMotionVecMaterial = null;
        Material m_ObjectMotionVecMaterial = null;
#if UNITY_GPU_DRIVEN_PIPELINE
        Material m_GpuMotionVectorMaterial = null;
        Material m_StencilSplatMaterial = null;
        Material m_DecalBufferEmitMaterial = null;
#endif
#if UNITY_EDITOR
        Material m_ShadingRateMaterial = null;
#endif
        ...
        {
            // Initialize k_DepthStencilFormat and k_DepthBufferBits.
            bool Is32BitsDepthBufferPlatform(RuntimePlatform platform)
            {
                switch (platform)
                {
                    ...
                    case RuntimePlatform.PlayableAdsPlayer:
                        return false;
            ...
            bool is32Bit = Application.isEditor || Is32BitsDepthBufferPlatform(Application.platform);

            k_DepthStencilFormat = is32Bit ? GraphicsFormat.D32_SFloat_S8_UInt : GraphicsFormat.D24_UNorm_S8_UInt;
            k_DepthBufferBits = is32Bit ? 32 : 24;
            ...
            m_DeferredPlusMaterial = CoreUtils.CreateEngineMaterial(data.shaders.deferredPlusPS);
...
#if UNITY_GPU_DRIVEN_PIPELINE
            m_GpuMotionVectorMaterial = CoreUtils.CreateEngineMaterial(data.shaders.gpuMotionVector);
            m_StencilSplatMaterial = CoreUtils.CreateEngineMaterial(data.shaders.stencilSplat);
            m_DecalBufferEmitMaterial = CoreUtils.CreateEngineMaterial(data.shaders.decalBufferEmit);
#endif
#if UNITY_EDITOR
            m_ShadingRateMaterial = CoreUtils.CreateEngineMaterial(data.shaders.shadingRate);
#endif
            ...
            forwardInitParams.forwardPlus = (data.renderingMode == RenderingMode.ForwardPlus || data.renderingMode == RenderingMode.DeferredPlus);
...
#if UNITY_GPU_DRIVEN_PIPELINE
            m_MainLightShadowCasterPass = new MainLightShadowCasterPass(RenderPassEvent.BeforeRenderingShadows, data.shaders.depthPyramid);
#else
            ...
            if (EnableGDRP())
            {
                m_GPUDrivenPass = new GPUDrivenPass(RenderPassEvent.BeforeRenderingPrePasses + 1);
                m_GPUReflectionProbePass = new GPUReflectionProbePass(RenderPassEvent.BeforeRenderingPrePasses + 2, data.shaders.reflectionProbeCulling);
                m_DecalBufferEmitPass = new DecalBufferEmitPass(RenderPassEvent.BeforeRenderingPrePasses + 3, m_DecalBufferEmitMaterial);
            }
#endif

#if UNITY_EDITOR
            m_ShadingRateOnlyPass = new ShadingRateOnlyPass(m_ShadingRateMaterial);
#endif
            ...
            if (this.renderingModeRequested == RenderingMode.Deferred || this.renderingModeRequested == RenderingMode.DeferredPlus)
            {
                ...
                deferredInitParams.deferredPlusMaterial = m_DeferredPlusMaterial;
                deferredInitParams.lightCookieManager = m_LightCookieManager;
                deferredInitParams.deferredPlus = (this.renderingModeRequested == RenderingMode.DeferredPlus);
                ...
                if (EnableGDRP())
                {
                    m_GBufferSplatPass = new GBufferSplatPass(RenderPassEvent.BeforeRenderingGbuffer - 2, RenderQueueRange.opaque, data.opaqueLayerMask, m_DeferredLights);
                    m_StencilSplatPass = new StencilSplatPass(RenderPassEvent.BeforeRenderingGbuffer - 1, m_StencilSplatMaterial);
                }
#endif
            ...
            if (EnableGDRP())
            {
                m_CopyDepthPyramidPass = new CopyDepthPyramidPass(copyDepthEvent);
                m_GPUMotionVectorPass = new GPUMotionVectorPass(copyDepthEvent + 1, m_GpuMotionVectorMaterial);
            }
#endif
            ...
            m_ColorBufferUpscaleSystem = new RenderTargetBufferSystem("_CameraColorAttachmentUpscale");

            supportedRenderingFeatures = new RenderingFeatures();

            if (this.renderingModeRequested is RenderingMode.Deferred or RenderingMode.DeferredPlus)
            {
...
#if UNITY_GPU_DRIVEN_PIPELINE
            m_GBufferSplatPass?.Dispose();
            m_GPUDrivenPass?.Dispose();
            m_GPUReflectionProbePass?.Dispose();
            m_GPUMotionVectorPass?.Dispose();
            m_StencilSplatPass?.Dispose();
#endif
        ...
        internal override void ReleaseRenderTargets()
        {
            ...
            m_ColorBufferUpscaleSystem.Dispose();
            ...
            m_ShadingRateColor?.Release();
            m_ShadingRateDepth?.Release();
            hasReleasedRTs = true;
        }
        ...
        private void SetupFinalPassDebug(ref CameraData cameraData)
        {
            ...
            {
#if UNITY_EDITOR
                if(DebugHandler.TryGetShowShadingRateMap(out bool showShadingRateMap))
                {
                    ShadingRateWindowManager.ToggleWindow(true);
                }
                else
                {
                    ShadingRateWindowManager.ToggleWindow(false);
                }
#endif
                if (DebugHandler.TryGetFullscreenDebugMode(out DebugFullScreenMode fullScreenDebugMode, out int textureHeightPercent) &&
                    (fullScreenDebugMode != DebugFullScreenMode.ReflectionProbeAtlas || isClusteredLighting))
                {
                    ...
                    switch (fullScreenDebugMode)
                    {
                        ...
                        case DebugFullScreenMode.ShadingRateValueMap:
                        {
                            DebugHandler.SetDebugRenderTarget(m_ShadingRateColor, normalizedRect, false);
                            break;
                        }
                        case DebugFullScreenMode.TJSR:
                        {
                            if (m_PostProcessPasses.postProcessPass.m_TjsrState?.intermediateRTs.tjsrDebugView != null)
                            {
                                DebugHandler.SetDebugRenderTarget(m_PostProcessPasses.postProcessPass.m_TjsrState.intermediateRTs.tjsrDebugView, normalizedRect, false);
                            }
                            break;
                        }
                        default:
...
#if UNITY_EDITOR
            else if (DebugHandler != null && !DebugHandler.TryGetShowShadingRateMap(out bool showShadingRateMap)) // Prevent window flickering
            {
                ShadingRateWindowManager.ToggleWindow(false);
            }
#endif
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
...
#if UNITY_GPU_DRIVEN_PIPELINE
                if (DebugHandler.GDRPDebugIsActive())
                {
                    DebugHandler.gdrpDebugPass.Setup(DebugHandler.DebugDisplaySettings.gdrpSettings.bufferVisualizationMode);
                    EnqueuePass(DebugHandler.gdrpDebugPass);
                }
#endif
            }
            ...
            var createColorTexture = ((rendererFeatures.Count != 0 && m_IntermediateTextureMode == IntermediateTextureMode.Always) && !isPreviewCamera) ||
                (Application.isEditor && isClusteredLighting);
            ...
            if (requiresRenderingLayer && !isDeferredLighting)
            {
            ...
            if (m_DeferredLights != null)
            {
                ...
                // When deferred+ is used, MixedLightingSetup is already resolved in ForwardLights, so we don't have
                // to do this again here
                // TODO: we'd better move all common logic into some file like m_ClusteredLights
                if (renderingModeActual == RenderingMode.DeferredPlus)
                    m_DeferredLights.SetMixedLightingMode(m_ForwardLights.MixedLightingSetup);
                else
...
#if UNITY_GPU_DRIVEN_PIPELINE
            m_MainLightShadowCasterPass.m_EnableGDRP = m_AdditionalLightsShadowCasterPass.m_EnableGDRP = EnableGDRP();
#endif
            ...
            if (requiresDepthPrepass && (this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus) && !renderPassInputs.requiresNormalsTexture)
            ...
            createDepthTexture |= ((this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus) && !useRenderPassEnabled);
            ...
            var colorUpscaleDescriptor = colorDescriptor;
            colorUpscaleDescriptor.width = cameraData.pixelWidth;
            colorUpscaleDescriptor.height = cameraData.pixelHeight;
            m_ColorBufferUpscaleSystem.SetCameraSettings(colorUpscaleDescriptor, FilterMode.Bilinear);
            // We should use m_ColorBufferSystem as default each frame and use
            // m_ColorBufferUpscaleSystem after upscale process.
            m_IsColorBufferUpscale = false;
            ...
            if (cameraData.renderType == CameraRenderType.Base)
            {
                ...
                m_ActiveCameraColorAttachment = createColorTexture ? colorBufferSystem.PeekBackBuffer() : m_XRTargetHandleAlias;
            ...
            else
            {
                ...
                if (m_ColorBufferUpscaleSystem != baseRenderer.m_ColorBufferUpscaleSystem)
                {
                    m_ColorBufferUpscaleSystem.Dispose();
                    m_ColorBufferUpscaleSystem = baseRenderer.m_ColorBufferUpscaleSystem;
                }
                m_IsColorBufferUpscale = baseRenderer.m_IsColorBufferUpscale;
                m_ActiveCameraColorAttachment = colorBufferSystem.PeekBackBuffer();
            ...
            if (rendererFeatures.Count != 0 && !isPreviewCamera)
                ConfigureCameraColorTarget(colorBufferSystem.PeekBackBuffer());
            ...
            if (this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus)
            {
            ...
            if (((this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus) && !this.useRenderPassEnabled) || requiresDepthPrepass || requiresDepthCopyPass)
            {
                var depthDescriptor = cameraTargetDescriptor;
                if ((requiresDepthPrepass && !isDeferredLighting) || !RenderingUtils.SupportsGraphicsFormat(GraphicsFormat.R32_SFloat, FormatUsage.Render))
                {
            ...
            if (requiresRenderingLayer || (isDeferredLighting && m_DeferredLights.UseRenderingLayers))
            {
                ref var renderingLayersTexture = ref m_DecalLayersTexture;
                string renderingLayersTextureName = "_CameraRenderingLayersTexture";

                if (isDeferredLighting && m_DeferredLights.UseRenderingLayers)
                {
                ...
                if (isDeferredLighting) // As this is requested by render pass we still want to set it
            ...
            if (requiresDepthPrepass && renderPassInputs.requiresNormalsTexture)
            {
                ...
                if (this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus) // As this is requested by render pass we still want to set it
            ...
            if (requiresDepthPrepass)
            {
                if (renderPassInputs.requiresNormalsTexture)
                {
                    if (this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus)
                    {
                        ...
                        {
                            if (m_DeferredLights.UseRenderingLayers)
                                m_DecalBufferEmitPass.Setup(true, m_ActiveCameraDepthAttachment, m_DeferredLights.GbufferAttachments[m_DeferredLights.GBufferRenderingLayers]);
                            else if(renderingLayerProvidesByDepthNormalPass)
                                m_DecalBufferEmitPass.Setup(true, m_ActiveCameraDepthAttachment, m_DecalLayersTexture);
                            else
                                m_DecalBufferEmitPass.Setup(false);
                        }
#endif
                    }
                    else
...
#if UNITY_GPU_DRIVEN_PIPELINE
                    if (EnableGDRP())
                        EnqueuePass(m_DecalBufferEmitPass);
#endif
                }
                else
                {
            ...
            if (EnableGDRP())
            {
                m_GPUDrivenPass.Setup(m_ActiveCameraDepthAttachment, ref renderingData, k_AfterFinalBlitPassQueueOffset);
                EnqueuePass(m_GPUDrivenPass);
                m_GPUReflectionProbePass.Setup(cmd, ref renderingData);
                EnqueuePass(m_GPUReflectionProbePass);
            }
#endif
            ...
            if (requiresDepthCopyPass && !((this.renderingModeActual == RenderingMode.Deferred || this.renderingModeActual == RenderingMode.DeferredPlus) && useRenderPassEnabled && !renderPassInputs.requiresDepthTexture))
            {
            ...
            if (EnableGDRP())
            {
                m_CopyDepthPyramidPass.Setup(m_ActiveCameraDepthAttachment);
                EnqueuePass(m_CopyDepthPyramidPass);
            }
#endif
            ...
            if (renderPassInputs.requiresMotionVectors)
            {
                ...
                if (EnableGDRP())
                {
                    m_GPUMotionVectorPass.Setup(m_MotionVectorColor, m_MotionVectorDepth);
                    EnqueuePass(m_GPUMotionVectorPass);
                }
#endif
            }

#if UNITY_EDITOR
            bool requiresShadingRateOnlyPass = (DebugHandler != null) && DebugHandler.IsActiveForCamera(ref cameraData);
            if (requiresShadingRateOnlyPass)
            {
                var colorDesc = cameraTargetDescriptor;
                colorDesc.graphicsFormat = ShadingRateOnlyPass.k_TargetFormat;
                colorDesc.depthBufferBits = (int)DepthBits.None;
                colorDesc.msaaSamples = 1;  // Disable MSAA, consider a pixel resolve for half left velocity and half right velocity --> no velocity, which is untrue.
                RenderingUtils.ReAllocateIfNeeded(ref m_ShadingRateColor, colorDesc, FilterMode.Point, TextureWrapMode.Clamp, name: "_ShadingRateTexture");

                var depthDescriptor = cameraTargetDescriptor;
                depthDescriptor.graphicsFormat = GraphicsFormat.None;
                depthDescriptor.msaaSamples = 1;
                RenderingUtils.ReAllocateIfNeeded(ref m_ShadingRateDepth, depthDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: "_ShadingRateDepthTexture");

                m_ShadingRateOnlyPass.Setup(m_ShadingRateColor, m_ShadingRateDepth);
                EnqueuePass(m_ShadingRateOnlyPass);
            }
#endif
            ...
            // 3. TJSR sharpening using standalone RCAS pass is required.
            // 4. TAA sharpening using standalone RCAS pass is required. (When upscaling is not enabled).
            bool applyFinalPostProcessing = anyPostProcessing && lastCameraInTheStack &&
                ((renderingData.cameraData.antialiasing == AntialiasingMode.FastApproximateAntialiasing) ||
                ((renderingData.cameraData.imageScalingMode == ImageScalingMode.Upscaling) &&
                (renderingData.cameraData.upscalingFilter == ImageUpscalingFilter.Point || renderingData.cameraData.upscalingFilter == ImageUpscalingFilter.FSR)));

            switch (cameraData.temporalMethod)
            {
                case TemporalMethod.TJSR:
                    applyFinalPostProcessing |= UniversalRenderPipeline.asset.tjsrSettings.tjsrContrastAdaptiveSharpening > 0.0f;
                    break;
                case TemporalMethod.TAA:
                    applyFinalPostProcessing |= renderingData.cameraData.taaSettings.contrastAdaptiveSharpening > 0.0f;
                    break;
        ...
        public override void SetupLights(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if (EnableGDRP())
            {
                m_GPUReflectionProbePass.SetupLights(ref renderingData);
            }
#endif
        }
            ...
            ref CameraData cameraData)
        {
            ...
            if (renderingModeActual == RenderingMode.ForwardPlus || renderingModeActual == RenderingMode.DeferredPlus)
            {
            ...
            else if (this.renderingModeActual == RenderingMode.DeferredPlus)
            {
                cullingParameters.maximumVisibleLights = 0xFFFF;
        ...
        public override void FinishRendering(CommandBuffer cmd)
        {
            ...
            m_ColorBufferUpscaleSystem.Clear();
            m_IsColorBufferUpscale = false;
...
#if UNITY_GPU_DRIVEN_PIPELINE
                if (EnableGDRP())
                    m_GBufferSplatPass.Configure(null, renderingData.cameraData.cameraTargetDescriptor);
#endif
...
#if UNITY_GPU_DRIVEN_PIPELINE
            if (EnableGDRP())
                EnqueuePass(m_GBufferSplatPass);
#endif
            EnqueuePass(m_GBufferPass);
#if UNITY_GPU_DRIVEN_PIPELINE
            if (EnableGDRP())
            {
                m_StencilSplatPass.Setup(m_ActiveCameraDepthAttachment);
                EnqueuePass(m_StencilSplatPass);
            }
#endif
        ...
        {
            RenderPassEvent beforeMainRenderingEvent = isDeferredLighting ? RenderPassEvent.BeforeRenderingGbuffer : RenderPassEvent.BeforeRenderingOpaques;
            ...
            {
                if (VolumeManager.instance.stack.GetComponent<MotionBlur>()?.mode.value == MotionBlurMode.CameraAndObjects)
            ...
            if (renderingData.cameraData.temporalMethod != TemporalMethod.None)
                inputSummary.requiresMotionVectors = true;
        ...
        void CreateCameraRenderTarget(ScriptableRenderContext context, ref RenderTextureDescriptor descriptor, bool primedDepth, CommandBuffer cmd, ref CameraData cameraData)
        {
            ...
            {
                if (colorBufferSystem.PeekBackBuffer() == null || colorBufferSystem.PeekBackBuffer().nameID != BuiltinRenderTextureType.CameraTarget)
                {
                    m_ActiveCameraColorAttachment = colorBufferSystem.GetBackBuffer(cmd);
...
#if UNITY_GPU_DRIVEN_PIPELINE
        public bool EnableGDRP()
        {
            return (GraphicsSettings.enableGDRP && renderingModeRequested == RenderingMode.Deferred);
        }
#endif
        internal override void SwapColorBuffer(CommandBuffer cmd)
        {
            ...
            if (m_ActiveCameraDepthAttachment.nameID != BuiltinRenderTextureType.CameraTarget)
                ConfigureCameraTarget(colorBufferSystem.GetBackBuffer(cmd), m_ActiveCameraDepthAttachment);
            else
                ConfigureCameraColorTarget(colorBufferSystem.GetBackBuffer(cmd));
        ...
        internal override void SetCameraColorBufferUpscale(bool upscale)
        {
            m_IsColorBufferUpscale = upscale;
        }
        
        internal override RTHandle GetCameraColorFrontBuffer(CommandBuffer cmd)
        {
            return colorBufferSystem.GetFrontBuffer(cmd);
        }

        internal override RTHandle GetCameraColorBackBuffer(CommandBuffer cmd)
        {
            return colorBufferSystem.GetBackBuffer(cmd);
        }

        internal override void EnableSwapBufferMSAA(bool enable)
        {
            colorBufferSystem.EnableMSAA(enable);
        }
    }
}

...
namespace UnityEngine.Rendering.Universal
{
    ...
    internal class PostProcessPass : ScriptableRenderPass
    {
        ...
        internal TJSR.TjsrRenderState m_TjsrState;
        ...
        Bloom m_Bloom;
        ScreenSpaceLensFlare m_ScreenSpaceLensFlare;
        ...
        bool m_UseRGBM; // (R, G, B, Multiplier)
        ...
        public void Dispose()
        {
            ...
            m_TjsrState?.intermediateRTs?.Dispose();
        }
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            m_ScreenSpaceLensFlare = stack.GetComponent<ScreenSpaceLensFlare>();
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            // Validate MetalFX
            bool useMetalFXSpatial = false;
            if (MetalFXUtilsURP.IsRequestedForCamera(ref cameraData))
            {
                string warning = MetalFXUtilsURP.ValidateAndWarn(ref cameraData);
                if (warning != null)
                {
                ...
                {
                    if (cameraData.upscalingFilter == ImageUpscalingFilter.MetalFXSpatial)
                        useMetalFXSpatial = true;
                }
            }
#endif

            cameraData.temporalMethod = ValidateTemporalMethod(ref cameraData);

            int amountOfPassesRemaining = (useStopNan ? 1 : 0) + (useSubPixeMorpAA ? 1 : 0) + (useDepthOfField ? 1 : 0) + (useLensFlare ? 1 : 0) + (cameraData.temporalMethod != TemporalMethod.None ? 1 : 0) + (useMotionBlur ? 1 : 0) + (usePaniniProjection ? 1 : 0);
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IOS
            amountOfPassesRemaining += (useMetalFXSpatial ? 1 : 0);
#endif
            ...
            if (useDepthOfField)
            {
                ...
                {
                    DoDepthOfField(ref cameraData, cmd, GetSource(), GetDestination(), cameraData.pixelRect);
                    Swap(ref renderer);
                }
            }

            // Temporal Anti Aliasing
            switch (cameraData.temporalMethod)
            {
                case TemporalMethod.TJSR:
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.TJSR)))
                    {
                        DoTJSR(cmd, ref cameraData, ref renderer, source);
                        Swap(ref renderer);
                    }
                    break;
                case TemporalMethod.TAA:
                    ...
                    break;
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IOS
                case TemporalMethod.MetalFXTemporal:
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.MetalFXUpscaling)))
                    {
                        DoMetalFX(cmd, ref cameraData, ref renderer, GetSource(), ImageUpscalingFilter.MetalFXTemporal);
                        Swap(ref renderer);
                    }
                    break;
#endif
...
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IOS
            // MetalFX Spatial
            if (useMetalFXSpatial)
            {
                using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.MetalFXUpscaling)))
                {
                    DoMetalFX(cmd, ref cameraData, ref renderer, GetSource(), ImageUpscalingFilter.MetalFXSpatial);
                    Swap(ref renderer);
                }
            }
#endif
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                if (bloomActive)
                {
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.Bloom)))
                        SetupBloom(cmd, GetSource(), m_Materials.uber, preserveAlpha: cameraData.preserveAlpha);
                }
                ...
                bool useScreenSpaceLensFlare = m_ScreenSpaceLensFlare.IsActive();
                if (bloomActive && useScreenSpaceLensFlare)
                {
                    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.ScreenSpaceLensFlare)))
                        SetupScreenSpaceLensFlare(cmd, GetSource(), m_Materials.uber, cameraData.camera);
                }
                ...
                if (cameraData.preserveAlpha)
                {
                    m_Materials.uber.EnableKeyword(ShaderKeywordStrings._PRESERVE_ALPHA);
                }
        ...
        #region TJSR
        // Use material to check if TJSR/TAA is supported.
        internal TemporalMethod CheckTemporalMethodIsSupported(TemporalMethod temporalMethod)
        {
            const int warningThrottleFrames = 60 * 1;
            string warning = null;
            switch (temporalMethod)
            {
                case TemporalMethod.TJSR:
                    if (null != m_Materials.tjsrMaterial)
                        return TemporalMethod.TJSR;
                    else
                        warning = "Disabling TJSR because Tuanjie Super Resolution Shader is not supported in this device.";
                    break;
                case TemporalMethod.TAA:
                    if (null != m_Materials.temporalAntialiasing)
                        return TemporalMethod.TAA;
                    else
                        warning = "Disabling TAA because TemporalAntialiasing Shader is not supported in this device.";
                    break;
                default:
        ...
        TemporalMethod ValidateTemporalMethod(ref CameraData cameraData)
        {
            if (cameraData.antialiasing != AntialiasingMode.TemporalAntiAliasing)
                return TemporalMethod.None;

            const int warningThrottleFrames = 60 * 1; // 60 FPS * 1 sec
            // TemporalUtils.ValidateAndWarn should have the same result as UniversalRenderPipeline::CheckTemporalMethod().
            string warning = TemporalUtils.ValidateAndWarn(ref cameraData);
            if (warning != null)
            {
                warning = "Disabling Temporal PostProcess(TAA and TJSR) " + warning;
                if (Time.frameCount % warningThrottleFrames == 0)
                    Debug.LogWarning(warning);
            ...
            switch (cameraData.temporalMethod)
            {
                case TemporalMethod.TJSR:
                    if (null == m_Materials.tjsrMaterial)
                        warning = "Disabling TJSR because Tuanjie Super Resolution Shader is not supported in this device.";
                    else if (cameraData.temporalPersistentData is not TjsrPersistentData)
                        warning = "Disabling TJSR because temporal persistent data is invalid.";
                ...
                case TemporalMethod.TAA:
                    if (null == m_Materials.temporalAntialiasing)
                        warning = "Disabling TAA because TemporalAntialiasing Shader is not supported in this device.";
                    else if (cameraData.temporalPersistentData is not TaaPersistentData)
                        warning = "Disabling TAA because temporal persistent data is invalid.";

                    if (warning == null)
                        return TemporalMethod.TAA;
                    break;
                case TemporalMethod.MetalFXTemporal:
                    if (cameraData.renderScale >= 0.5f)
                        return TemporalMethod.MetalFXTemporal;

                    // Render scale should already be checked during initialization
                    Debug.LogError("MetalFX TAAU requires render scale >= 0.5. Should already fallback to MetalFXSpatial at this phase. Please check camera initialization logic.");
                    break;
                case TemporalMethod.None:
                default:
                    Debug.LogError("PostProcess TemporalMethod is invalid! TemporalUtils.ValidateAndWarn should have the same result as UniversalRenderPipeline::CheckTemporalMethod().");
                    break;
            }
        ...
        void DoTJSR(CommandBuffer cmd, ref CameraData cameraData, ref ScriptableRenderer renderer, RTHandle source)
        {
            UpdateScreenSize(cmd, new Vector2Int(cameraData.pixelWidth, cameraData.pixelHeight));
            renderer.SetCameraColorBufferUpscale(true);
            var destination = renderer.GetCameraColorFrontBuffer(cmd);
            // HDR output
            bool requireHDROutput = RequireHDROutput(ref cameraData);
            if (requireHDROutput)
            {
                SetupHDROutput(cameraData.hdrDisplayInformation, cameraData.hdrDisplayColorGamut, m_Materials.tjsrMaterial, HDROutputUtils.Operation.None);
            }
            // Lazy initialization. Use Lazy<> ?
            if (null == m_TjsrState)
                m_TjsrState = new();
            TjsrUtils.ExecutePass(cmd, m_Materials.tjsrMaterial, ref cameraData, source, destination, m_MotionVectors.rt, ref m_TjsrState);
            UpdateCameraResolution(ref cameraData, new Vector2Int(cameraData.pixelWidth, cameraData.pixelHeight));
        }
        
        void UpdateScreenSize(CommandBuffer cmd, Vector2Int newCameraTargetSize)
        {
            // Update the shader constants to reflect the new camera resolution
            cmd.SetGlobalVector(ShaderPropertyId.screenSize,
                new Vector4(
                    newCameraTargetSize.x,
                    newCameraTargetSize.y,
                    1.0f / newCameraTargetSize.x,
                    1.0f / newCameraTargetSize.y
                )
            );
        }
        
        void UpdateCameraResolution(ref CameraData cameraData, Vector2Int newCameraTargetSize)
        {
...
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX || UNITY_IOS
        void DoMetalFX(CommandBuffer cmd, ref CameraData cameraData, ref ScriptableRenderer renderer, RTHandle source, ImageUpscalingFilter filter)
        {
            UpdateCameraResolution(ref cameraData, new Vector2Int(cameraData.pixelWidth, cameraData.pixelHeight));
            UpdateScreenSize(cmd, new Vector2Int(cameraData.pixelWidth, cameraData.pixelHeight));
            renderer.SetCameraColorBufferUpscale(true);
            ...
            tempRtDesc.forMetalFXOutput = true;
            tempRtDesc.msaaSamples = 1;
            tempRtDesc.depthBufferBits = 0;
            tempRtDesc.width = cameraData.pixelWidth;
            tempRtDesc.height = cameraData.pixelHeight;
            tempRtDesc.enableRandomWrite = true;
            RenderingUtils.ReAllocateIfNeeded(ref m_UpscaledTarget, tempRtDesc, FilterMode.Point, TextureWrapMode.Clamp, name: "_UpscaledTexture");

            if (filter == ImageUpscalingFilter.MetalFXSpatial)
            {
                MetalFXCoreUtils.PerformMetalFXSpatialUpscaling(cmd, source, m_UpscaledTarget, cameraData.camera);
            }
            else  // MetalFXTemporal
            {
                Vector2 jitter = MetalFXUtilsURP.CalculateJitter(ref cameraData);
                // Call native method for MetalFX temporal upscaling
                MetalFXCoreUtils.PerformMetalFXTemporalUpscaling(cmd,
                    source,
                    m_UpscaledTarget,
                    m_Depth,
                    m_MotionVectors,
            ...
            {
                cmd.CopyTexture(m_UpscaledTarget, destination);
            }
            else
            {
                // CopyTexture() won't show up in frame debugger, use Blit() instead
                Blitter.BlitCameraTexture(cmd, m_UpscaledTarget, destination);
            }
#else
            cmd.CopyTexture(m_UpscaledTarget, destination);
#endif
        ...
        void DoDepthOfField(ref CameraData cameraData, CommandBuffer cmd, RTHandle source, RTHandle destination, Rect pixelRect)
        {
            if (m_DepthOfField.mode.value == DepthOfFieldMode.Gaussian)
                DoGaussianDepthOfField(cameraData.camera, cmd, source, destination, pixelRect, cameraData.preserveAlpha);
            else if (m_DepthOfField.mode.value == DepthOfFieldMode.Bokeh)
                DoBokehDepthOfField(cmd, source, destination, pixelRect, cameraData.preserveAlpha);
        }

        void DoGaussianDepthOfField(Camera camera, CommandBuffer cmd, RTHandle source, RTHandle destination, Rect pixelRect, bool preserveAlpha)
        {
            int downSample = 2;
            ...
            CoreUtils.SetKeyword(material, ShaderKeywordStrings._PRESERVE_ALPHA, preserveAlpha);
            ...
            RenderingUtils.ReAllocateIfNeeded(ref m_PingTexture, GetCompatibleDescriptor(wh, hh, m_DefaultHDRFormat), FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_PingTexture");
            RenderingUtils.ReAllocateIfNeeded(ref m_PongTexture, GetCompatibleDescriptor(wh, hh, m_DefaultHDRFormat), FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_PongTexture");
        ...
        void DoBokehDepthOfField(CommandBuffer cmd, RTHandle source, RTHandle destination, Rect pixelRect, bool preserveAlpha)
        {
            int downSample = 2;
            ...
            // Unlike GaussianDOF, HDR precision is not followed here, because we're storing coc in alpha channel.
    ...
    #region Motion Blur
        ...
        void DoMotionBlur(CommandBuffer cmd, RTHandle source, RTHandle destination, ref CameraData cameraData)
        {
            ...
            CoreUtils.SetKeyword(material, ShaderKeywordStrings._PRESERVE_ALPHA, cameraData.preserveAlpha);
            if (m_MotionBlur.mode.value == MotionBlurMode.CameraOnly)
            {
                ...
                Blitter.BlitCameraTexture(cmd, source, destination, RenderBufferLoadAction.DontCare,
                    RenderBufferStoreAction.Store, material, (int)m_MotionBlur.quality.value);
            }
            else if (m_MotionBlur.mode.value == MotionBlurMode.CameraAndObjects)
            {
                int objectMotionBlurPassIdx = (int)MotionBlurQuality.High + 1;

                material.SetFloat("_Intensity", m_MotionBlur.intensity.value);
                material.SetFloat("_Clamp", m_MotionBlur.clamp.value);
                material.SetTexture("_ObjectMotionBlurTex", m_MotionVectors.rt);
                PostProcessUtils.SetSourceSize(cmd, m_Descriptor);
                Blitter.BlitCameraTexture(cmd, source, destination, RenderBufferLoadAction.DontCare,
                    RenderBufferStoreAction.Store, material, objectMotionBlurPassIdx + (int)m_MotionBlur.quality.value);
            }
...
#region Bloom
        ...
        void SetupBloom(CommandBuffer cmd, RTHandle source, Material uberMaterial, bool preserveAlpha = false)
        {
            // Start at half-res
            int downres = 1;
            ...
            CoreUtils.SetKeyword(bloomMaterial, ShaderKeywordStrings._PRESERVE_ALPHA, preserveAlpha);
...
#region ScreenSpaceLensFlare
        void SetupScreenSpaceLensFlare(CommandBuffer cmd, RTHandle source, Material uberMaterial, Camera camera)
        {
            ...
            if (camera.cameraType == CameraType.SceneView)
            {
                ...
                for (int i = 0; i < UnityEditor.SceneView.sceneViews.Count; i++)
                {
                    ...
                    if (sv.camera == camera && !sv.sceneViewState.flaresEnabled)
                    {
                        return;
                    }
                }
            ...
            // Get the material used for the Screen Space Lens Flare.
            // Set keyword to indicate if the RGBM is used.
            var SSLFMaterial = m_Materials.screenSpaceLensFlare;
            CoreUtils.SetKeyword(SSLFMaterial, ShaderKeywordStrings.UseRGBM, m_UseRGBM);

            // Normalize tint color to isolate chromaticity from luminance variations
            var tint = m_ScreenSpaceLensFlare.tintColor.value.linear;
            var luma = ColorUtils.Luminance(tint);
            tint = luma > 0f ? tint * (1f / luma) : Color.white;

            // Maintains resolution-independent glare scaling.
            float glareLength = m_ScreenSpaceLensFlare.length.value;
            glareLength *= m_Descriptor.width * 0.0005f;

            // Halo scale
            float haloScaleX = 1.0f / m_ScreenSpaceLensFlare.haloScale.value.x * ((float)m_Descriptor.width / (float)m_Descriptor.height);
            float haloScaleY = 1.0f / m_ScreenSpaceLensFlare.haloScale.value.y;

            // Glare Orientation
            float radius = Mathf.Deg2Rad * m_ScreenSpaceLensFlare.orientation.value;
            float orientaionCos = Mathf.Cos(radius);
            float orientaionSin = Mathf.Sin(radius);

            // Setup Screen Space Lens Flare parameters
            SSLFMaterial.SetColor(ShaderConstants._ScreenSpaceLensFlare_TintColor, tint);
            SSLFMaterial.SetVector(ShaderConstants._ScreenSpaceLensFlare_Params1,
                new Vector4(m_ScreenSpaceLensFlare.intensity.value * 0.1f, m_ScreenSpaceLensFlare.regularMultiplier.value, m_ScreenSpaceLensFlare.reversedMultiplier.value, m_ScreenSpaceLensFlare.haloMultiplier.value));
            SSLFMaterial.SetVector(ShaderConstants._ScreenSpaceLensFlare_Params2,
                new Vector4(haloScaleX, haloScaleY, m_ScreenSpaceLensFlare.samples.value, m_ScreenSpaceLensFlare.sampleDimmer.value));
            SSLFMaterial.SetVector(ShaderConstants._ScreenSpaceLensFlare_Params3,
                new Vector4(m_ScreenSpaceLensFlare.startingPoint.value, m_ScreenSpaceLensFlare.ghostScale.value, 1.0f / m_ScreenSpaceLensFlare.vignetteScale.value.x, 1.0f / m_ScreenSpaceLensFlare.vignetteScale.value.y));
            SSLFMaterial.SetVector(ShaderConstants._ScreenSpaceLensFlare_Params4,
                new Vector4(Mathf.Pow(m_ScreenSpaceLensFlare.vignetteIntensity.value, 0.05f), m_ScreenSpaceLensFlare.chromatic.value * 0.05f, m_ScreenSpaceLensFlare.glareMultiplier.value * 4f, 0f));
            SSLFMaterial.SetVector(ShaderConstants._ScreenSpaceLensFlare_Params5,
                new Vector4(glareLength, orientaionCos, orientaionSin, Mathf.GammaToLinearSpace(m_ScreenSpaceLensFlare.glareThreshold.value)));

            // Achieve bloom upsample texture as input.
            // Not allow to use m_BloomMipUp[0] as input, as it will be used as output target in composition pass.
            var bloomMip = Mathf.Clamp(m_ScreenSpaceLensFlare.bloomMipBias.value, 1, m_Bloom.maxIterations.value / 2);
            var bloomMipUpTexture = m_BloomMipUp[bloomMip];

            // Draw Glares. Composition of the Pre-filter pass and the Blur pass.
            if (m_ScreenSpaceLensFlare.IsGlareActive())
            {
                // Create RT _ScreenSpaceLensFlare_GlareTexture to draw glares.

                int downScale = (int) m_ScreenSpaceLensFlare.resolution.value;
                int tw = m_Descriptor.width >> downScale;
                int th = m_Descriptor.height >> downScale;

                var desc = GetCompatibleDescriptor(tw, th, m_DefaultHDRFormat);
                RenderingUtils.ReAllocateIfNeeded(ref m_GlareTexture, desc, FilterMode.Bilinear, TextureWrapMode.Clamp, name: "_ScreenSpaceLensFlare_GlareTexture");

                int maxLevel = Mathf.FloorToInt(Mathf.Log(Mathf.Max(m_Descriptor.width, m_Descriptor.height), 2.0f));
                int maxLevelDownsample = Mathf.Max(1, maxLevel);
                int maxLevelUpsample = 2;

                Blitter.BlitCameraTexture(cmd, bloomMipUpTexture, m_GlareTexture, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, SSLFMaterial, 0);

                // Do Ping-pong shading.
                RTHandle bloomMipDownTexture = m_BloomMipDown[downScale - 1];
                for (int i = 0; i < maxLevelDownsample; i++)
                {
                    float size = ((float)i + 1) / Mathf.Pow(2, downScale) * 10f;
                    cmd.SetGlobalFloat(ShaderConstants._ScreenSpaceLensFlare_MipLevel, size);
                    Blitter.BlitCameraTexture(cmd, m_GlareTexture, bloomMipDownTexture, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, SSLFMaterial, 1);
                    CoreUtils.Swap(ref m_GlareTexture, ref bloomMipDownTexture);
                }
                for (int i = 0; i < maxLevelUpsample; i++)
                {
                    float size = ((float)i + 1) / Mathf.Pow(2, downScale) * 15f;
                    cmd.SetGlobalFloat(ShaderConstants._ScreenSpaceLensFlare_MipLevel, size);
                    Blitter.BlitCameraTexture(cmd, m_GlareTexture, bloomMipDownTexture, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, SSLFMaterial, 2);
                    CoreUtils.Swap(ref m_GlareTexture, ref bloomMipDownTexture);
                }

                // In case the RT _ScreenSpaceLensFlare_GlareTexture leaks. 
                m_BloomMipDown[downScale - 1] = bloomMipDownTexture;

                // pass Glares texture to uberpost material.
                SSLFMaterial.SetTexture(ShaderConstants._ScreenSpaceLensFlare_GlareTexture, m_GlareTexture);
            }

            // Composition + Chromatic Aberration
            Blitter.BlitCameraTexture(cmd, bloomMipUpTexture, m_BloomMipUp[0], RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, SSLFMaterial, 3);
        }
    #endregion
...
#region Final pass
        ...
        void RenderFinalPass(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            if (cameraData.preserveAlpha)
                material.EnableKeyword(ShaderKeywordStrings._PRESERVE_ALPHA);
            ...
            // Reuse RCAS pass as an optional standalone post sharpening pass for TAA and TJSR.
            ...
            if (!isFsrEnabled)
            {
                ...
                {
                    case TemporalMethod.TJSR:
                        temporalSharpness = UniversalRenderPipeline.asset.tjsrSettings.tjsrContrastAdaptiveSharpening;
                        break;
                    case TemporalMethod.TAA:
                        temporalSharpness = cameraData.taaSettings.contrastAdaptiveSharpening;
                        break;
                    case TemporalMethod.None:
                    default:
                        break;
                ...
                switch (cameraData.imageScalingMode)
                {
                    case ImageScalingMode.Upscaling:
                    {
                        ...
                        switch (cameraData.upscalingFilter)
                        {
                            ...
                            case ImageUpscalingFilter.TJSR:
                            {
                                break;
                            }
                            case ImageUpscalingFilter.MetalFXSpatial:
                            case ImageUpscalingFilter.MetalFXTemporal:
                                break;
                        }
            ...
            if (0.0f != temporalSharpness)
            {
                ...
                FSRUtils.SetRcasConstantsLinear(cmd, temporalSharpness);
            }
...
#region Internal utilities
        ...
        class MaterialLibrary
        {
            ...
            public readonly Material screenSpaceLensFlare;
            ...
            public readonly Material tjsrMaterial;
            ...
            public MaterialLibrary(PostProcessData data)
            {
                ...
                screenSpaceLensFlare = Load(data.shaders.screenSpaceLensFlarePS);
                ...
                tjsrMaterial = Load(data.shaders.tjsrPS);
            }
            ...
            internal void Cleanup()
            {
                ...
                CoreUtils.Destroy(screenSpaceLensFlare);
                ...
                CoreUtils.Destroy(tjsrMaterial);
            }
        }
        ...
        static class ShaderConstants
        {
            ...
            public static readonly int _ScreenSpaceLensFlare_GlareTexture = Shader.PropertyToID("_ScreenSpaceLensFlare_GlareTexture");
            public static readonly int _ScreenSpaceLensFlare_MipLevel = Shader.PropertyToID("_ScreenSpaceLensFlare_MipLevel");
            public static readonly int _ScreenSpaceLensFlare_TintColor = Shader.PropertyToID("_ScreenSpaceLensFlare_TintColor");
            public static readonly int _ScreenSpaceLensFlare_Params1 = Shader.PropertyToID("_ScreenSpaceLensFlare_Params1");
            public static readonly int _ScreenSpaceLensFlare_Params2 = Shader.PropertyToID("_ScreenSpaceLensFlare_Params2");
            public static readonly int _ScreenSpaceLensFlare_Params3 = Shader.PropertyToID("_ScreenSpaceLensFlare_Params3");
            public static readonly int _ScreenSpaceLensFlare_Params4 = Shader.PropertyToID("_ScreenSpaceLensFlare_Params4");
            public static readonly int _ScreenSpaceLensFlare_Params5 = Shader.PropertyToID("_ScreenSpaceLensFlare_Params5");

...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public partial class AdditionalLightsShadowCasterPass : ScriptableRenderPass
    {
        ...
        private List<bool> m_OnlyVGShadowSlices = new List<bool>();
#endif
...
#if UNITY_GPU_DRIVEN_PIPELINE
                m_ShadowSetting.shadowViews = new GPUDriven.VGShadowView[maxVisibleAdditionalLights];
#endif
            }
...
#if UNITY_GPU_DRIVEN_PIPELINE
                                    if (m_EnableGDRP && m_DrawShadowGDRP)
                                    {
                                        m_OnlyVGShadowSlices.Add(shadowCastersBounds.center.Equals(Vector3.zero) && shadowCastersBounds.size.Equals(Vector3.zero));
                                    }
#endif
                                }
                            }
        ...
        void RenderAdditionalShadowmapAtlas(ref ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.AdditionalLightsShadow)))
            {
                ...
                if (m_EnableGDRP && m_DrawShadowGDRP)
                {
                    ...
                    m_ShadowSetting.enablePostCull = false;
                    ...
                    m_ShadowSetting.shadowmapRT = m_AdditionalLightsShadowmapHandle;
                    m_ShadowSetting.viewSizeAndInvSize = new Vector4(renderTargetWidth, renderTargetHeight, 1.0f / renderTargetWidth, 1.0f / renderTargetHeight);
                    if (m_UseStructuredBuffer)
                        m_ShadowSetting.shadowViews = new GPUDriven.VGShadowView[shadowSlicesCount];
                }
#endif
                for (int globalShadowSliceIndex = 0; globalShadowSliceIndex < shadowSlicesCount; ++globalShadowSliceIndex)
                {
                    ...
                    if (!m_EnableGDRP || !m_DrawShadowGDRP || !m_OnlyVGShadowSlices[globalShadowSliceIndex])
#endif
                    {
                    ...
                    if (m_EnableGDRP && m_DrawShadowGDRP)
                    {
                        ref var shadowView = ref m_ShadowSetting.shadowViews[globalShadowSliceIndex];
                        ...
                        shadowView.viewport = new Vector4(shadowSliceData.offsetX, shadowSliceData.offsetY, shadowSliceData.resolution, shadowSliceData.resolution);
                        shadowView.viewMatrix = shadowSliceData.viewMatrix;
                        shadowView.projMatrix = shadowSliceData.projectionMatrix;
                        // There's no need to use pre frame data for culling without post.
                        shadowView.preViewMatrix = shadowSliceData.viewMatrix;
                        shadowView.preProjMatrix = shadowSliceData.projectionMatrix;
                    }
#endif
                }
                ...
                if (m_EnableGDRP && m_DrawShadowGDRP)
                {
                    if (!m_UseStructuredBuffer)
                    {
                        for (int viewIndex = m_ShadowSliceToAdditionalLightIndex.Count;
                             viewIndex < m_ShadowSetting.shadowViews.Length;
                             ++viewIndex)
                        {

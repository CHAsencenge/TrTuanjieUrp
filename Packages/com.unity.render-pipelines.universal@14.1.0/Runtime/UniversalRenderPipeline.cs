...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        // We cannot allocate new memory during rendering, so we shall allocate this when starting.
        private static Vector4[] m_AmbientProbeData = new Vector4[7];
        ...
        private void DisposeAdditionalCameraData()
        {
            foreach (var c in Camera.allCameras)
            {
                if (c.TryGetComponent<UniversalAdditionalCameraData>(out var acd))
                {
                    ...
                    acd.tjsrPersistentData?.DeallocateTargets();
                };
            }
        }
        ...
        static void RenderSingleCamera(ScriptableRenderContext context, ref CameraData cameraData)
        {
...
#if UNITY_GPU_DRIVEN_PIPELINE && UNITY_EDITOR
            if (GraphicsSettings.enableGDRP && renderer is UniversalRenderer universalRenderer)
            {
                if (universalRenderer.renderingModeActual == RenderingMode.Forward ||
                    universalRenderer.renderingModeActual == RenderingMode.ForwardPlus ||
                    universalRenderer.renderingModeActual == RenderingMode.DeferredPlus)
                {
                    RenderPipelineManager.isNeedURPWarning = true;
                }
            }
#endif
            if (renderer == null)
            ...
            using (new ProfilingScope(cmdScope, sampler)) // Enqueues a "BeginSample" command into the CommandBuffer cmd
            {
                ...
                // Update temporal persistent data based on cameraData. Most importantly resize the history render targets.
                // NOTE: Persistent data is kept over multiple frames. Its life-time differs from typical resources.
                // NOTE: Shared between both Execute and Render (RG) paths.
                if (cameraData.temporalPersistentData != null)
                    cameraData.temporalPersistentData.UpdateTargets(ref renderingData.cameraData);
#if ADAPTIVE_PERFORMANCE_2_0_0_OR_NEWER
                if (asset.useAdaptivePerformance)
        ...
        static TemporalMethod CheckTemporalMethod(ref CameraData cameraData, UniversalAdditionalCameraData additionalCameraData)
        {
            bool IsTemporalPPEnabled = (cameraData.antialiasing == AntialiasingMode.TemporalAntiAliasing)                                // Enabled
                   && (cameraData.cameraTargetDescriptor.msaaSamples == 1)                                                               // No MSAA
                   && !(additionalCameraData?.renderType == CameraRenderType.Overlay || additionalCameraData?.cameraStack.Count > 0)     // No Camera stack
                   && !cameraData.camera.allowDynamicResolution                                                                          // No Dynamic Resolution
                   && cameraData.postProcessEnabled;                                                                                     // No Postprocessing

            var render = additionalCameraData?.scriptableRenderer as UniversalRenderer;
            var postPro = render?.postProcessPass;

            if (!IsTemporalPPEnabled || postPro == null)
                return TemporalMethod.None;

            if (cameraData.imageScalingMode == ImageScalingMode.Upscaling)
            {
                if (cameraData.upscalingFilter == ImageUpscalingFilter.TJSR)
                {
                    // Need to check TJSR is supported when initializing CameraData.
                    return postPro.CheckTemporalMethodIsSupported(TemporalMethod.TJSR);
                }
                else if (cameraData.upscalingFilter == ImageUpscalingFilter.MetalFXTemporal)
                {
                    if (cameraData.renderScale >= 0.5f)
                        return TemporalMethod.MetalFXTemporal;

                    Debug.LogWarning($"MetalFX temporal not enabled because the minimum render scale is 0.5 (now {cameraData.renderScale}). Fallback to MetalFX Spatial.");
                    cameraData.upscalingFilter = ImageUpscalingFilter.MetalFXSpatial;
                }
            }

            // Need to check TAA is supported when initializing CameraData.
            return postPro.CheckTemporalMethodIsSupported(TemporalMethod.TAA);
        }
        ...
        static bool CheckPostProcessForDepth(ref CameraData cameraData)
        {
            ...
            if ((cameraData.antialiasing == AntialiasingMode.SubpixelMorphologicalAntiAliasing || cameraData.temporalMethod != TemporalMethod.None)
        ...
        static void InitializeCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData, bool resolveFinalTarget, out CameraData cameraData)
        {
            ...
            cameraData.preserveAlpha = GraphicsFormatUtility.HasAlphaChannel(cameraData.cameraTargetDescriptor.graphicsFormat);
        }
        ...
        static void InitializeStackedCameraData(Camera baseCamera, UniversalAdditionalCameraData baseAdditionalCameraData, ref CameraData cameraData)
        {
            ...
            else if ((cameraData.renderScale < 1.0f) || (!isScenePreviewOrReflectionCamera && (cameraData.upscalingFilter == ImageUpscalingFilter.FSR)))
            {
                ...
                // When TJSR or MetalFX temporal is requested, we force temporal anti-aliasing on since it's a prerequisite.
                if (cameraData.upscalingFilter is ImageUpscalingFilter.TJSR or ImageUpscalingFilter.MetalFXTemporal)
                {
        ...
        static void InitializeAdditionalCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData, bool resolveFinalTarget, ref CameraData cameraData)
        {
...
#if UNITY_GPU_DRIVEN_PIPELINE
                cameraData.vgShadowData = additionalCameraData.vgShadowData;
#endif
            }
            else
            {
            ...
            cameraData.temporalMethod = CheckTemporalMethod(ref cameraData, additionalCameraData);
            ...
            // NOTE: Temporal PostProcess depends on XR modifications of cameraTargetDescriptor.
            if (additionalCameraData != null)
                UpdateTemporalPPData(ref cameraData, additionalCameraData);
            ...
            Matrix4x4 jitterMat = TemporalUtils.CalculateJitterMatrix(ref cameraData);
            ...
            cameraData.preserveAlpha = cameraData.preserveAlpha && (!cameraData.postProcessEnabled || settings.postProcessPreserveAlpha);
        }
            ...
            CommandBuffer cmd, out RenderingData renderingData)
        {
            ...
            var isForwardPlus = cameraData.renderer is UniversalRenderer { renderingModeActual: RenderingMode.ForwardPlus }
                || cameraData.renderer is UniversalRenderer { renderingModeActual: RenderingMode.DeferredPlus };
...
#pragma warning restore 0618
        ...
        private static void UpdateTemporalPPData(ref CameraData cameraData, UniversalAdditionalCameraData additionalCameraData)
        {
            switch (cameraData.temporalMethod)
            {
                case TemporalMethod.TJSR:
                    if (additionalCameraData.tjsrPersistentData == null)
                    {
                        bool xrMultipassEnabled = false;
#if ENABLE_VR && ENABLE_XR_MODULE
                        xrMultipassEnabled = cameraData.xr.enabled && !cameraData.xr.singlePassEnabled;
#endif
                        additionalCameraData.tjsrPersistentData = new TjsrPersistentData(xrMultipassEnabled);
                    }
                    // When TJSR is active, we don't require the full set of resources needed by TAA.
                    additionalCameraData.taaPersistentData?.DeallocateTargets();
                    cameraData.temporalPersistentData = additionalCameraData.tjsrPersistentData;
                    break;
                case TemporalMethod.TAA:
                    if (additionalCameraData.taaPersistentData == null)
                    {
                        additionalCameraData.taaPersistentData = new TaaPersistentData();
                    }
                    // When TAA is active, need to release TJSR persistent resources.
                    additionalCameraData.tjsrPersistentData?.DeallocateTargets();
                    // Initialize shared TAA target desc.
                    ref var desc = ref cameraData.cameraTargetDescriptor;
                    additionalCameraData.taaPersistentData.Init(desc.width, desc.height, desc.volumeDepth, desc.graphicsFormat, desc.vrUsage, desc.dimension);
                    cameraData.temporalPersistentData = additionalCameraData.taaPersistentData;
                    break;
        ...
        static void SetupPerCameraShaderConstants(CommandBuffer cmd)
        {
            ...
            // For VG, we need set some buffer for LightProbes
            ambientSH.GetShaderConstantsFromNormalizedSH(m_AmbientProbeData);
            cmd.SetGlobalVectorArray(ShaderPropertyId.ambientProbeData, m_AmbientProbeData);
        }
        ...
        static ImageUpscalingFilter ResolveUpscalingFilterSelection(Vector2 imageSize, float renderScale, UpscalingFilterSelection selection)
        {
            ...
            var advancedUpscalingFilter = asset.ResolveAdvancedUpscalingFilterOption();
            if (advancedUpscalingFilter != AdvancedUpscalingFilterOption.None)
            {
                switch (advancedUpscalingFilter)
                {
                    case AdvancedUpscalingFilterOption.MetalFX:
                        return asset.metalFXMode == MetalFXModeSelection.Spatial
                            ? ImageUpscalingFilter.MetalFXSpatial
                            : ImageUpscalingFilter.MetalFXTemporal;
                    default:
                        break;
                }
            }
            ...
            switch (selection)
            {
                ...
                case UpscalingFilterSelection.TJSR:
                {
                    filter = ImageUpscalingFilter.TJSR;

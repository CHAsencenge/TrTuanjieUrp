...
#define SHADERGRAPH_BAKED_GI(positionWS, normalWS, positionSS, uvStaticLightmap, uvDynamicLightmap, applyScaling, instance, materialOffset, ddx, ddy) shadergraph_LWBakedGI(positionWS, normalWS, positionSS, uvStaticLightmap, uvDynamicLightmap, applyScaling, instance, ddx, ddy)
#define SHADERGRAPH_GLOBALILLUMINATION(albedo, metallic,specular,smoothness, clearCoatMask,clearCoatSmoothness,shadowMask,BakeGI,occlusion,positionWS,normalWS,viewDirectionWS,instance) shadergraph_LWGlobalIllumination(albedo, metallic,specular,smoothness,clearCoatMask,clearCoatSmoothness,shadowMask,BakeGI,occlusion,positionWS,normalWS,viewDirectionWS,instance)
#else
...
#define SHADERGRAPH_GLOBALILLUMINATION(albedo, metallic,specular,smoothness, clearCoatMask,clearCoatSmoothness,shadowMask,BakeGI,occlusion,positionWS,normalWS,viewDirectionWS) shadergraph_LWGlobalIllumination(albedo, metallic,specular,smoothness,clearCoatMask,clearCoatSmoothness,shadowMask,BakeGI,occlusion,positionWS,normalWS,viewDirectionWS)
#endif

#define SHADERGRAPH_MAINLIGHTING(Albedo,Metallic,Specular,Smoothness,Occlusion,Alpha,shadowMask,normalWS,positionWS,viewDirectionWS,clearCoatMask,clearCoatSmoothness,specularHighlightsOff)shadergraph_LWMainLighting(Albedo,Metallic,Specular,Smoothness,Occlusion,Alpha,shadowMask,normalWS,positionWS,viewDirectionWS,clearCoatMask,clearCoatSmoothness,specularHighlightsOff)
#define SHADERGRAPH_ADDITIONALLIGHTING(Albedo,Metallic,Specular,Smoothness,Occlusion,Alpha,shadowMask,normalWS,positionWS,viewDirectionWS,clearCoatMask,clearCoatSmoothness,specularHighlightsOff)shadergraph_LWAdditionalLighting(Albedo,Metallic,Specular,Smoothness,Occlusion,Alpha,shadowMask,normalWS,positionWS,viewDirectionWS,clearCoatMask,clearCoatSmoothness,specularHighlightsOff)
...
#ifdef UNITY_GPU_DRIVEN_PIPELINE
#define SHADERGRAPH_SHADOWMASK(LightmapUV, instance, ddx, ddy) shadergraph_LWShadowMask(LightmapUV, instance, ddx, ddy)
#define SHADERGRAPH_MAINLIGHTSHADOW(positionWS, LightmapUV, instance, ddx, ddy) shadergraph_LWMainLightShadow(positionWS, LightmapUV, instance, ddx, ddy)
#else
...
float3 shadergraph_LWBakedGI(float3 positionWS, float3 normalWS, uint2 positionSS, float2 uvStaticLightmap, float2 uvDynamicLightmap, bool applyScaling UNITY_GDRP_INSTANCE_PARAMETER UNITY_GDRP_DDXDDY_PARAMETER)
{
#ifdef LIGHTMAP_ON
    if (applyScaling)
    {
    ...
    return SampleLightmap(uvStaticLightmap, uvDynamicLightmap, normalWS UNITY_GDRP_DDXDDY_ARGUMENT);
#else
    return SampleLightmap(uvStaticLightmap, normalWS UNITY_GDRP_DDXDDY_ARGUMENT);
#endif
#else
    return SampleSH(normalWS, positionWS UNITY_GDRP_INSTANCE_ARGUMENT);
#endif
}

float3 shadergraph_LWGlobalIllumination(float3 albedo, float metallic, float3 specular, float smoothness, float clearCoatMask, float clearCoatSmoothness,
    half4 shadowMask, half3 BakeGI, half occlusion, float3 positionWS,
    half3 normalWS, half3 viewDirectionWS UNITY_GDRP_INSTANCE_PARAMETER)
{
    ...
    half2 normalizedScreenSpaceUV = GetNormalizedScreenSpaceUV(positionCS.xy);
    ...
    MixRealtimeAndBakedGI(mainLight, normalWS, BakeGI);

    return GlobalIllumination(brdfData, brdfDataClearCoat, clearCoatMask, BakeGI, occlusion, positionWS, normalWS, viewDirectionWS, normalizedScreenSpaceUV UNITY_GDRP_INSTANCE_ARGUMENT);

}

float3 shadergraph_LWMainLighting(float3 Albedo,float Metallic,float3 Specular, float Smoothness,
    float Occlusion,float Alpha,float4 shadowMask,float3 normalWS,float3 positionWS,float3 viewDirectionWS,
    float clearCoatMask,float clearCoatSmoothness,bool specularHighlightsOff)
{
    float3 Out = half3(1, 0, 0);
    ...
    float4 positionCS=TransformWorldToHClip(positionWS);
    float2 normalizedScreenSpaceUV=GetNormalizedScreenSpaceUV(positionCS.xy);
    AmbientOcclusionFactor aoFactor = CreateAmbientOcclusionFactor(normalizedScreenSpaceUV, Occlusion);
    ...
    #endif
         {
             Out = LightingPhysicallyBased(brdfData, brdfDataClearCoat,
                                                              mainLight,
 ...
 float3 shadergraph_LWAdditionalLighting(float3 Albedo,float Metallic,float3 Specular,
     float Smoothness,float Occlusion,float Alpha,float4 shadowMask,float3 normalWS,float3 positionWS,
     float3 viewDirectionWS,float clearCoatMask,float clearCoatSmoothness, bool specularHighlightsOff)
{
    float3 Out = half3(0, 0, 0);
    BRDFData brdfData;
    InitializeBRDFData(Albedo, Metallic, Specular, Smoothness, Alpha, brdfData);
    ...
    for (uint lightIndex = 0; lightIndex < min(URP_FP_DIRECTIONAL_LIGHTS_COUNT, MAX_VISIBLE_LIGHTS); lightIndex++)
    {
    ...
    #endif
        {
            ...
            Out += LightingPhysicallyBased(brdfData, brdfDataClearCoat, light,
                                                                         normalWS, viewDirectionWS,
                                                                          clearCoatMask, specularHighlightsOff);
        }
    }
    ...
    // #if defined(_ADDITIONAL_LIGHTS_VERTEX)
     //  Out += inputData.vertexLighting * brdfData.diffuse;
    //  #endif
...
#if USE_CLUSTERED_LIGHTING || defined(UNITY_GPU_DRIVEN_PIPELINE)
...
float4 shadergraph_LWShadowMask(float2 LightmapUV UNITY_GDRP_INSTANCE_PARAMETER UNITY_GDRP_DDXDDY_PARAMETER)
{
    // To ensure backward compatibility we have to avoid using shadowMask input, as it is not present in older shaders
#if defined(SHADOWS_SHADOWMASK) && defined(LIGHTMAP_ON)
    OUTPUT_LIGHTMAP_UV(LightmapUV, unity_LightmapST, LightmapUV);
#ifdef UNITY_GPU_DRIVEN_PIPELINE
    half4 shadowMask = SAMPLE_SHADOWMASK(LightmapUV, ddx, ddy);
#else
...
float shadergraph_LWMainLightShadow(float3 positionWS, float2 LightmapUV UNITY_GDRP_INSTANCE_PARAMETER UNITY_GDRP_DDXDDY_PARAMETER)
{
    float4 shadowCoord;
#if defined(_MAIN_LIGHT_SHADOWS_SCREEN) && !defined(_SURFACE_TYPE_TRANSPARENT)
    shadowCoord = ComputeScreenPos(TransformWorldToHClip(positionWS));
    ...
    OUTPUT_LIGHTMAP_UV(LightmapUV, unity_LightmapST, LightmapUV);
#ifdef UNITY_GPU_DRIVEN_PIPELINE
    float4 shadowMask = SAMPLE_SHADOWMASK(LightmapUV, ddx, ddy);
#else

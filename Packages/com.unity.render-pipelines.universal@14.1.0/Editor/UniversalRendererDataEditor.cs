...
namespace UnityEditor.Rendering.Universal
{
    ...
    public class UniversalRendererDataEditor : ScriptableRendererDataEditor
    {
        private static class Styles
        {
            ...
            public static readonly GUIContent invalidStencilOverride = EditorGUIUtility.TrTextContent("Error: When using the deferred / deferred+ rendering path, the Renderer requires the control over the 4 highest bits of the stencil buffer to store Material types. The current combination of the stencil override options prevents the Renderer from controlling the required bits. Try changing one of the options to Replace.");
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                var newValue = m_RenderingMode.intValue;
                TuanjieGRDAnalytics.SendChangeRenderingModeEvent("Change rendering mode", ((RenderingMode)newValue).ToString());
            }
            ...
            bool isDeferred =
                m_RenderingMode.intValue is (int)RenderingMode.Deferred or (int)RenderingMode.DeferredPlus;
            if (overrideStencil.boolValue && isDeferred)
            {

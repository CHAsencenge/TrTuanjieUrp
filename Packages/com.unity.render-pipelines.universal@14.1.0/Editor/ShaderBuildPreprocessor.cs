...
namespace UnityEditor.Rendering.Universal
{
    ...
    enum ShaderFeatures : long
    {
        ...
        PreserveAlpha = (1L << 49),
        UseLightmapSingle = (1L << 50),
        BRGVariantsStorageTexture = (1L << 51),
        BRGVariantsStorageBoth = (1L << 52),
        DeferredPlus = (1L << 53),
        TJSRLow = (1L << 54),
        TJSRMedium = (1L << 55),
        TJSRHigh = (1L << 56),
    ...
    class ShaderBuildPreprocessor : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        internal struct RendererRequirements
        {
            ...
            public bool needsDeferred => (renderingMode == RenderingMode.Deferred || renderingMode == RenderingMode.DeferredPlus);
            public bool needsClusteredLighting => (renderingMode == RenderingMode.ForwardPlus || renderingMode == RenderingMode.DeferredPlus);
        }
        ...
        private static void GetSupportedFeaturesFromVolumes(ref VolumeFeatures volumeFeatures)
        {
            ...
            foreach (string guid in guids)
            {
                ...
                if (asset.Has<ScreenSpaceLensFlare>())
                    s_VolumeFeatures |= VolumeFeatures.ScreenSpaceLensFlare;
            ...
            out bool everyRendererHasSSAO)
        {
            ...
            if (urpAsset.postProcessPreserveAlpha)
                urpAssetShaderFeatures |= ShaderFeatures.PreserveAlpha;

            if (urpAsset.gpuResidentDrawerStat == UniversalRenderPipelineAsset.GPUResidentDrawerStat.InstancedDrawing)
                urpAssetShaderFeatures |= ShaderFeatures.UseLightmapSingle;

            // if (urpAsset.gpuResidentDrawerStat == UniversalRenderPipelineAsset.GPUResidentDrawerStat.InstancedDrawing)
            // {
            //     urpAssetShaderFeatures |= ShaderFeatures.BRGVariantsStorageTexture;
            // }

            switch(urpAsset.brgShaderVariantStorageStripType)
            {
                case UniversalRenderPipelineAsset.BRGShaderVariantStorageStripType.KeepTexture:
                    urpAssetShaderFeatures |= ShaderFeatures.BRGVariantsStorageTexture;
                    break;
                case UniversalRenderPipelineAsset.BRGShaderVariantStorageStripType.KeepSSBOOrUniform:
                    break;
                case UniversalRenderPipelineAsset.BRGShaderVariantStorageStripType.KeepAll:
                    urpAssetShaderFeatures |= ShaderFeatures.BRGVariantsStorageBoth;
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }

            if (urpAsset.upscalingFilter == UpscalingFilterSelection.TJSR)
            {
                switch (urpAsset.tjsrSettings.tjsrQuality)
                {
                    case TJSRQuality.Low:
                        urpAssetShaderFeatures |= ShaderFeatures.TJSRLow;
                        break;
                    case TJSRQuality.Medium:
                        urpAssetShaderFeatures |= ShaderFeatures.TJSRMedium;
                        break;
                    case TJSRQuality.High:
                        urpAssetShaderFeatures |= ShaderFeatures.TJSRHigh;
                        break;
                    default:
                        throw new ArgumentOutOfRangeException();
                }
        ...
        internal static RendererRequirements GetRendererRequirements(ref UniversalRenderPipelineAsset urpAsset, ref ScriptableRenderer renderer, ref ScriptableRendererData rendererData)
        {
            ...
            rsd.needsAdditionalLightShadows       = urpAsset.supportsAdditionalLightShadows && (urpAsset.additionalLightsRenderingMode == LightRenderingMode.PerPixel || rsd.needsClusteredLighting);
            ...
            rsd.needsGBufferRenderingLayers       = (rsd.isUniversalRenderer && rsd.needsDeferred && urpAsset.useRenderingLayers);
            rsd.needsGBufferAccurateNormals       = (rsd.isUniversalRenderer && rsd.needsDeferred && universalRenderer.accurateGbufferNormals);
            rsd.needsRenderPass                   = (rsd.isUniversalRenderer && rsd.needsDeferred && universalRenderer.useRenderPassEnabled);
        ...
        internal static ShaderFeatures GetSupportedShaderFeaturesFromRenderer(ref RendererRequirements rendererRequirements, ref ScriptableRendererData rendererData, ref List<ScreenSpaceAmbientOcclusionSettings> ssaoRendererFeatures, ref bool containsForwardRenderer, ShaderFeatures urpAssetShaderFeatures)
        {
            ...
            // Forward+ or Deferred+
            if (rendererRequirements.needsClusteredLighting)
            {
                shaderFeatures |= ShaderFeatures.AdditionalLightsKeepOffVariants;
        ...
        internal static ShaderFeatures GetSupportedShaderFeaturesFromRendererFeatures(ref RendererRequirements rendererRequirements, ref List<ScriptableRendererFeature> rendererFeatures, ref List<ScreenSpaceAmbientOcclusionSettings> ssaoRendererFeatures)
        {
            ...
            bool isDeferredRenderer = rendererRequirements.needsDeferred;
            ...
            )
        {
            ...
            spd.stripPreserveAlphaKeywords = !IsFeatureEnabled(shaderFeatures, ShaderFeatures.PreserveAlpha);
            ...
            spd.clusteredLightingPrefilteringMode = PrefilteringMode.Remove;
            if (isAssetUsingForwardPlus || isAssetUsingDeferredPlus)
            {
                ...
                if (!isAssetUsingForward && !isAssetUsingDeferred)
                    spd.clusteredLightingPrefilteringMode = PrefilteringMode.SelectOnly;
                else
                    spd.clusteredLightingPrefilteringMode = PrefilteringMode.Select;
            }
            ...
            spd.useLightmapSingle = IsFeatureEnabled(shaderFeatures, ShaderFeatures.UseLightmapSingle);

            spd.brgVariantStorageStripMode = PrefilteringMode.Remove;
            if (IsFeatureEnabled(shaderFeatures, ShaderFeatures.BRGVariantsStorageTexture))
            {
                spd.brgVariantStorageStripMode = PrefilteringMode.SelectOnly;
            }
            if (IsFeatureEnabled(shaderFeatures, ShaderFeatures.BRGVariantsStorageBoth))
            {
                spd.brgVariantStorageStripMode = PrefilteringMode.Select;
            }

...
namespace UnityEditor.Rendering.Universal
{
    ...
    internal class ShaderScriptableStripper : IShaderVariantStripper, IShaderVariantStripperScope
    {
        ...
        Shader m_DeferredPlusShading = Shader.Find("Hidden/Universal Render Pipeline/Deferred+ Shading");
        ...
        Shader m_TJSRShader = Shader.Find("Hidden/Universal Render Pipeline/TJSR");
        ...
        LocalKeyword m_TJSRActive;
        LocalKeyword m_TJSRIntermediateColor;
        LocalKeyword m_TJSRDebugView;
        ...
        private void InitializeLocalShaderKeywords([DisallowNull] Shader shader)
        {
            ...
            m_TJSRActive = TryGetLocalKeyword(shader, TJSR.ShaderKeywords.TJSR_HAS_ACTIVE_PASS);
            m_TJSRIntermediateColor = TryGetLocalKeyword(shader, TJSR.ShaderKeywords.TJSR_INTERMEDIATE_COLOR);
            m_TJSRDebugView = TryGetLocalKeyword(shader, TJSR.ShaderKeywords.TJSR_DEBUGVIEW);
        ...
        *********************************************************/
        ...
        internal bool StripUnusedFeatures_TJSR(ref IShaderScriptableStrippingData strippingData, ref ShaderStripTool<ShaderFeatures> stripTool)
        {
            // If this is not the right shader, then skip
            if (strippingData.shader != m_TJSRShader)
                return false;

            if (!strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRHigh) &&
                !strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRMedium) &&
                !strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRLow))
            {
                return true;
            }

            if (strippingData.stripDebugDisplayShaders)
            {
                if (strippingData.IsKeywordEnabled(m_TJSRDebugView) || strippingData.passName == TJSR.ShaderPassResources.kPassNameTJSRDebugView)
                    return true;
            }

            if (!strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRHigh))
            {
                // Strip the pass unused in Medium and Low quality level.
                if (strippingData.passName == TJSR.ShaderPassResources.kPassNameTJSRSetup || strippingData.passName == TJSR.ShaderPassResources.kPassNameTJSRActive)
                    return true;

                // Strip the keyword not enabled in Medium quality level.
                if (stripTool.StripMultiCompile(m_TJSRIntermediateColor, ShaderFeatures.TJSRMedium))
                    return true;
            }

            if (!strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRMedium) &&
                !strippingData.IsShaderFeatureEnabled(ShaderFeatures.TJSRLow))
            {
                // Strip the pass unused in High quality level.
                if (strippingData.passName == TJSR.ShaderPassResources.kPassNameTJSRReconstruct)
                    return true;

                // Strip the keyword not enabled in High quality level.
                if (stripTool.StripMultiCompile(m_TJSRIntermediateColor, ShaderFeatures.TJSRHigh))
                    return true;
            }

            // Strip the keyword of Active Pass.
            if (stripTool.StripMultiCompile(m_TJSRActive, ShaderFeatures.TJSRHigh))
                return true;
        ...
        *********************************************************/
        ...
        internal bool StripUnusedFeatures_ClusteredLighting(ref IShaderScriptableStrippingData strippingData)
        {
            if (!strippingData.IsShaderFeatureEnabled(ShaderFeatures.ForwardPlus) 
                && !strippingData.IsShaderFeatureEnabled(ShaderFeatures.DeferredPlus))
            {
                if (strippingData.IsKeywordEnabled(m_ForwardPlus))
                    return true;
            }
            else if (strippingData.stripUnusedVariants)
            {
                if (!strippingData.IsKeywordEnabled(m_ForwardPlus) && strippingData.PassHasKeyword(m_ForwardPlus))
                    return true;
            }
            return false;
        }
        ...
        {
            // Forward+/Deferred+ doesn't use Vertex or the Pixel Light variants.
        ...
        internal bool StripUnusedFeatures(ref IShaderScriptableStrippingData strippingData)
        {
            ...
            if (StripUnusedFeatures_ClusteredLighting(ref strippingData))
                return true;
            ...
            if (StripUnusedFeatures_TJSR(ref strippingData, ref stripTool))
                return true;
        ...
        *********************************************************/
        ...
        {
            // Strip Additional Shadow variants if it's not set to PerPixel and not F+/Deferred/Deferred+
        ...
        *********************************************************/
        ...
        internal bool StripUnusedShaders_Deferred(ref IShaderScriptableStrippingData strippingData)
        {
            ...
            else if (strippingData.shader == m_DeferredPlusShading)
            {

            ...
            for (int rendererIndex = 0; rendererIndex < rendererDataList.Length; ++rendererIndex)
            {
                ...
                foreach (var feature in rendererFeatures)
                    if (feature is ScreenSpaceAmbientOcclusion && (ScreenSpaceAmbientOcclusion)feature == ssaoFeature)
                        return rendererData is UniversalRendererData && (
                            ((UniversalRendererData)rendererData).renderingMode == RenderingMode.Deferred
                            || ((UniversalRendererData)rendererData).renderingMode == RenderingMode.DeferredPlus);
            }

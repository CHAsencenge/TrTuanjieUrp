...
Texture2D<uint4> _VisibilityBuffer;  // for VG objects
...
void InitializeInputData(Varyings input, float3 positionWS, half3 normalWS, half3 viewDirectionWS, out InputData inputData)
{
    ...
    // ShaderPassDecal.hlsl just is used in Shader Graph with DecalPass.template, and all passes, including this file,
    // will define "VARYINGS_NEED_DYNAMIC_LIGHTMAP_UV" and "VARYINGS_NEED_STATIC_LIGHTMAP_UV".
...
#if defined(DECAL_LOAD_NORMAL)
half3 GetVGNormalWS(float2 uv, half3 normalWS)
{
    ...
    if (clusterID != 0xFFFFFFFF)
    {
        ...
        SurvivalCluster SurvivalCluster = LoadSurvivalCluster(clusterID); // _VisibleClustersSWHW
        ...
        // early out if material does not receive decals
        uint index = GetRelativeMaterialIndex(cluster, triangleID);
        int materialOffset = _MaterialSlotBuffer[instance.materialSlotOffset + index * 2 + 1];
        ...
        const float2 pixelClip = uv * _ScreenSize.zw * float2(2, 2) + float2(-1, -1);
        ...
        PixelAttribute attributeData = GetPixelAttribute(cluster, triIndices, barycentrics, pageClusterCount, (float3x3)local2WorldMatrix, (float3x3)world2LocalMatrix);

        normalWS = attributeData.normalWS;
    }
...
)
{
    ...
    normalWS = GetVGNormalWS(positionCS.xy, normalWS);
#endif

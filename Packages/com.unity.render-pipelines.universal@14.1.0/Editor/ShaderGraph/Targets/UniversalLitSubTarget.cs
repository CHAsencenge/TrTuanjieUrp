...
{
    sealed class UniversalLitSubTarget : UniversalSubTarget, ILegacyTarget, IVGSubTarget
    {
        ...
        private LitTargetParams m_subTargetParams;
        ...
        public override void CollectShaderProperties(PropertyCollector collector, GenerationMode generationMode)
        {
            ...
            if (GraphicsSettings.enableGDRP && !target.allowMaterialOverride)
            {
        ...
        {
            // Some nodes and feature could not be compatible with Virtual Geometry.
            for (int i = 0; i < (int)GraphData.UncompatibleNodeType.UncompatibleCount; i++)
            {
                if ((target.uncompatibleGDRP & (1u << i)) != 0)
                    context.AddHelpBox(MessageType.Info, $"{(GraphData.UncompatibleNodeType)i} is not compatible with Virtual Geometry in Lit target.");
            }
            // If the Clear Coat feature is enabled, GBuffer pass cannot be generated. Accordingly, GBufferSplat pass
            // also does not exist. So we should info the message to users.
            if (clearCoat)
                context.AddHelpBox(MessageType.Info, "The Clear Coat is not compatible with Virtual Geometry in Lit target.");
        ...
        public bool SupportsGDRP()
        {
            return GraphicsSettings.enableGDRP && clearCoat == false;
        }
        ...
        {
            public static SubShaderDescriptor LitSubShader(UniversalTarget target, LitTargetParams targetParams, WorkflowMode workflowMode, string renderType, string renderQueue, string disableBatchingTag, bool complexLit, bool blendModePreserveSpecular)
            {
                ...
                if (complexLit && targetParams.passStatus[PassName.GetPassID(PassName.forwardPass)])
                    result.passes.Add(LitPasses.ForwardOnly(target, targetParams, workflowMode, complexLit, blendModePreserveSpecular, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, CorePragmas.Forward, LitKeywords.Forward));
                else if(targetParams.passStatus[PassName.GetPassID(PassName.forwardPass)])
                    result.passes.Add(LitPasses.Forward(target, targetParams, workflowMode, blendModePreserveSpecular, CorePragmas.Forward, LitKeywords.Forward));
                ...
                if (targetParams.passStatus[PassName.GetPassID(PassName.gBufferPass)])
                    result.passes.Add(LitPasses.GBuffer(target, targetParams, workflowMode, blendModePreserveSpecular));

                if (GraphicsSettings.enableGDRP && target.uncompatibleGDRP == 0)
                {
                    if (!complexLit && targetParams.passStatus[PassName.GetPassID(PassName.gBufferSplatPass)])
                    {
                        result.passes.Add(LitPasses.GBufferSplat(target, targetParams, workflowMode, blendModePreserveSpecular));
                        if (target.alphaClip)
                        {
                            result.passes.Add(LitPasses.VBufferAlphaClip(target));
                            result.passes.Add(LitPasses.AlphaMeshSelection(target));
                            result.passes.Add(LitPasses.VGShadowMapAlphaClipSingleView(target));
                            result.passes.Add(LitPasses.VGShadowMapAlphaClipMultiView(target));
                        }
                    }
                }
                ...
                if (complexLit && targetParams.passStatus[PassName.GetPassID(PassName.depthNormalPass)])
                    result.passes.Add(PassVariant(LitPasses.DepthNormalOnly(target, targetParams), CorePragmas.Instanced));
                else if (targetParams.passStatus[PassName.GetPassID(PassName.depthNormalPass)])
                    result.passes.Add(PassVariant(LitPasses.DepthNormal(target, targetParams), CorePragmas.Instanced));
        ...
        {
            static void AddKeywordsControlToForwardPass(ref PassDescriptor passDesc, LitTargetParams targetParams)
            {
            ...
            static void AddKeywordsControlToGBufferPass(ref PassDescriptor passDesc, LitTargetParams targetParams)
            {
            ...
            public static PassDescriptor GBuffer(UniversalTarget target, LitTargetParams targetParams, WorkflowMode workflowMode, bool blendModePreserveSpecular)
            {
            ...
            public static PassDescriptor GBufferSplat(UniversalTarget target, LitTargetParams targetParams, WorkflowMode workflowMode, bool blendModePreserveSpecular)
            {
                var result = new PassDescriptor
                {
                    // Definition
                    displayName = "GBufferSplat",
                    referenceName = "SHADERPASS_GBUFFER_SPLAT",
                    lightMode = "UniversalGBufferSplat",
                    useInPreview = false,
                    isVirtualGeometry = true,

                    // Template
                    passTemplatePath = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Templates/GBufferSplatPass.template",
                    sharedTemplateDirectories = UniversalTarget.kSharedTemplateDirectories,
                    ...
                    // Fields
                    structs = CoreStructCollections.Default,
                    requiredFields = LitRequiredFields.GBufferSplat,
                    fieldDependencies = CoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CoreRenderStates.GBufferSplat,
                    pragmas = CorePragmas.GBufferSplat,
                    defines = new DefineCollection { CoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection { LitKeywords.GBufferSplat },
                    includes = new IncludeCollection { LitIncludes.GBufferSplat },

                    // Custom Interpolator Support
                    customInterpolators = CoreCustomInterpDescriptors.Common,
                };

                // @TODO: Check if reusing GBuffer interface is correct
                AddKeywordsControlToGBufferPass(ref result, targetParams);
                CorePasses.AddTargetSurfaceControlsToPass(ref result, target, blendModePreserveSpecular);
                AddWorkflowModeControlToPass(ref result, target, workflowMode);
                AddReceiveShadowsControlToPass(ref result, target, target.receiveShadows);
            ...
            public static PassDescriptor VBufferAlphaClip(UniversalTarget target)
            {
                var result = new PassDescriptor
                {
                    // Definition
                    displayName = "VBuffer",
                    referenceName = "SHADERPASS_VBUFFER_ALPHA_CLIP",
                    lightMode = "VBufferAlphaClip",
                    useInPreview = false,
                    isVirtualGeometry = true,

                    // Template
                    passTemplatePath = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Templates/AlphaClipPass.template",
                    sharedTemplateDirectories = UniversalTarget.kSharedTemplateDirectories,
                    ...
                    // Fields
                    structs = CoreStructCollections.Default,
                    requiredFields = LitRequiredFields.AlphaClip,
                    fieldDependencies = CoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CoreRenderStates.VBufferAlphaClip,
                    pragmas = CorePragmas.AlphaClip,
                    defines = new DefineCollection {},
                    keywords = new KeywordCollection {},
                    includes = new IncludeCollection { LitIncludes.VBufferAlphaClip },
            ...
            public static PassDescriptor AlphaMeshSelection(UniversalTarget target)
            {
                var result = new PassDescriptor
                {
                    // Definition
                    displayName = "MeshSelection",
                    referenceName = "SHADERPASS_ALPHA_MESH_SELECTION",
                    lightMode = "AlphaMeshSelection",
                    useInPreview = false,
                    isVirtualGeometry = true,
                    ...
                    // Conditional State
                    renderStates = CoreRenderStates.AlphaMeshSelection,
                    pragmas = CorePragmas.AlphaClip,
                    defines = new DefineCollection {},
                    keywords = new KeywordCollection {},
                    includes = new IncludeCollection { LitIncludes.AlphaMeshSelection },
            ...
            private static PassDescriptor VGShadowMapAlphaClip(UniversalTarget target)
            {
                var result = new PassDescriptor
                {
                    // Definition
                    displayName = "VGShadowMap",
                    referenceName = "SHADERPASS_VGSHADOWMAP_ALPHA_CLIP",
                    lightMode = "VGShadowMapAlphaClip",
                    useInPreview = false,
                    isVirtualGeometry = true,
                    ...
                    // Conditional State
                    renderStates = CoreRenderStates.VGShadowMapAlphaClip,
                    pragmas = CorePragmas.AlphaClip,
                    defines = new DefineCollection {},
                    keywords = new KeywordCollection { CoreKeywords.ShadowCaster }, // Add shadow bias keywords
                    includes = new IncludeCollection { LitIncludes.VGShadowMapAlphaClip },
            ...
            public static PassDescriptor VGShadowMapAlphaClipSingleView(UniversalTarget target)
            {
                var result = VGShadowMapAlphaClip(target);
                result.displayName += "SingleView";
                return result;
            }

            public static PassDescriptor VGShadowMapAlphaClipMultiView(UniversalTarget target)
            {
                var result = VGShadowMapAlphaClip(target);
                result.displayName += "MultiView";
                result.defines.Add(CoreKeywordDescriptors.MultiView, 1);
                return result;
            }
            ...
            public static PassDescriptor DepthNormal(UniversalTarget target, LitTargetParams targetParams)
            {
                var result = new PassDescriptor()
                {
                    // Definition
            ...
            public static PassDescriptor DepthNormalOnly(UniversalTarget target, LitTargetParams targetParams)
            {
                var result = new PassDescriptor()
                {
                    // Definition
        ...
        static class LitRequiredFields
        {
            ...
            public static readonly FieldCollection GBufferSplat = new FieldCollection()
            {
                StructFields.Varyings.positionWS,
                StructFields.Varyings.normalWS,
            ...
            // If we support Vertex Animation, we should add variables in need here.
            public static readonly FieldCollection AlphaClip = new FieldCollection()
            {
                UniversalStructFields.Varyings.sh,
            };
        ...
        static class LitKeywords
        {
            ...
            public static readonly KeywordCollection GBufferSplat = new KeywordCollection
            {
        ...
        static class LitIncludes
        {
            ...
            const string kPBRGBufferSplatPass = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Includes/PBRGBufferSplatPass.hlsl";
            ...
            const string kPBRVisibilityPass = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Includes/PBRVisibilityPass.hlsl";
            const string kPBRAlphaMeshSelectionPass = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Includes/PBRAlphaMeshSelectionPass.hlsl";
            const string kPBRVGShadowMapPass = "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Includes/PBRVGShadowMap.hlsl";
            ...
            public static readonly IncludeCollection GBufferSplat = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.WriteRenderLayersPregraph },
                { CoreIncludes.GDRPPregraph },
                { kShadows, IncludeLocation.Pregraph },
                { CoreIncludes.ShaderGraphPregraph },
                { CoreIncludes.DBufferPregraph },
                ...
                { CoreIncludes.CorePostgraph },
                { kGBuffer, IncludeLocation.Postgraph },
                { kPBRGBufferSplatPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection VBufferAlphaClip = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.GDRPPregraph },
                { CoreIncludes.ShaderGraphPregraph },
                ...
                // Post-graph
                { CoreIncludes.CorePostgraph },
                { kPBRVisibilityPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection AlphaMeshSelection = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.GDRPPregraph },
                { CoreIncludes.ShaderGraphPregraph },
                ...
                // Post-graph
                { CoreIncludes.CorePostgraph },
                { kPBRAlphaMeshSelectionPass, IncludeLocation.Postgraph },
            };

            public static readonly IncludeCollection VGShadowMapAlphaClip = new IncludeCollection
            {
                // Pre-graph
                { CoreIncludes.GDRPPregraph },
                { CoreIncludes.ShaderGraphPregraph },
                ...
                // Post-graph
                { CoreIncludes.CorePostgraph },
                { kPBRVGShadowMapPass, IncludeLocation.Postgraph },
            };

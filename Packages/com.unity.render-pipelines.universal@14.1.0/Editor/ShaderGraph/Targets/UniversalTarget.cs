...
namespace UnityEditor.Rendering.Universal.ShaderGraph
{
    ...
    public enum MaterialType
    {
        ...
        /// <summary>
        /// Use this for URP scalable lit.
        /// </summary>
        ScalableLit,

        /// <summary>
        /// Use this for URP fabric.
        /// </summary>
        Fabric,
    }
...
#endif
    {
        ...
        [SerializeField]
        public int m_StencilReference = 0;
        [SerializeField]
        public int m_StencilReadMask = 0;
        [SerializeField]
        public int m_StencilWriteMask = 0;
        [SerializeField]
        public CompareFunction m_StencilCompareFunction = CompareFunction.Always;
        [SerializeField]
        public StencilOp m_PassOperation = StencilOp.Keep;
        [SerializeField]
        public StencilOp m_FailOperation = StencilOp.Keep;
        [SerializeField]
        public StencilOp m_ZFailOperation = StencilOp.Keep;
        ...
        private static readonly List<Type> m_CompatibleVGSubTarget = new List<Type>
        {
            typeof(UniversalLitSubTarget)
        };
        ...
        public static bool HasFeatureType(Enum feature, Enum featureType) => feature.HasFlag(featureType);

        public override int CustomPassCount() => m_ActiveSubTarget.value.CustomPassCount();
        public override List<int> GetCustomPassType() => m_ActiveSubTarget.value.GetCustomPassType();
        ...
        {
            get => m_PassOperation;
            set => m_PassOperation = value;
        }
        public StencilOp failOperation
        {
            get => m_FailOperation;
            set => m_FailOperation = value;
        }
        public StencilOp zFailOperation
        {
            get => m_ZFailOperation;
            set => m_ZFailOperation = value;
        }
        ...
        {
            get => additionalMotionVectorMode != AdditionalMotionVectorMode.None;
        }
        ...
        public override void GetPropertiesGUI(ref TargetPropertyGUIContext context, Action onChange, Action<String> registerUndo)
        {
            ...
            context.AddProperty("Material", m_SubTargetField, (evt) =>
            {
                ...
                ProcessSubTargetDatas(m_ActiveSubTarget.value);
                onChange();
                ShaderGraphGUIAnalytics.SendShaderGraphCreate("ChangeUniversalTarget", m_ActiveSubTarget.value.displayName);
            });

            if (!m_CompatibleVGSubTarget.Contains(m_ActiveSubTarget.value.GetType()))
                context.AddHelpBox(MessageType.Info, $"The {m_ActiveSubTarget.value.displayName} target does not support Virtual Geometry.");
        ...
        public void AddDefaultSurfacePropertiesGUI(ref TargetPropertyGUIContext context, Action onChange, Action<String> registerUndo, bool showReceiveShadows)
        {
            ...
            context.AddProperty("Override Stencil State", new Toggle() { value = overrideStencilState }, (evt) =>
            {
                if (Equals(overrideStencilState, evt.newValue))
                    return;
                registerUndo("Change Override Stencil State");
            ...
            if (overrideStencilState)
            {
                ...
                var ReferenceControl = new IntegerField("") { value = stencilReference };
                ReferenceControl.RegisterCallback<FocusOutEvent>(
                evt =>
                {
                    if (Equals(stencilReference, ReferenceControl.value))
                        return;
                    registerUndo("Change Stencil Reference");
                    stencilReference = Math.Clamp(ReferenceControl.value, 0, 255);
                    onChange();
                });
                context.AddProperty("Reference", ReferenceControl, evt=> { });

                var ReadMaskControl = new IntegerField("") { value = stencilReadMask };
                ReadMaskControl.RegisterCallback<FocusOutEvent>(
                evt =>
                {
                    if (Equals(stencilReadMask, ReadMaskControl.value))
                        return;
                    registerUndo("Change Stencil Read Mask");
                    stencilReadMask = Math.Clamp(ReadMaskControl.value, 0, 255);
                    onChange();
                });
                context.AddProperty("Read Mask", ReadMaskControl, evt=> { });

                var WriteMaskControl = new IntegerField("") { value = stencilWriteMask };
                WriteMaskControl.RegisterCallback<FocusOutEvent>(
                evt =>
                {
                    if (Equals(stencilWriteMask, WriteMaskControl.value))
                        return;
                    registerUndo("Change Stencil Write Mask");
                    stencilWriteMask = Math.Clamp(WriteMaskControl.value, 0, 255);
                    onChange();
                });
                context.AddProperty("Write Mask", WriteMaskControl, evt=> { });

                context.AddProperty("Compare", new EnumField(CompareFunction.Always) { value = stencilCompareFunction }, (evt) =>
                {
                    if (Equals(stencilCompareFunction, evt.newValue))
                        return;
                    registerUndo("Change Stencil Compare Function");
                    stencilCompareFunction = (CompareFunction)evt.newValue;
                    onChange();
                });

                context.AddProperty("Pass", new EnumField(StencilOp.Keep) { value = passOperation }, (evt) =>
                {
                    if (Equals(passOperation, evt.newValue))
                        return;
                    registerUndo("Change Stencil Pass Operation");
                    passOperation = (StencilOp)evt.newValue;
                    onChange();
                });

                context.AddProperty("Fail", new EnumField(StencilOp.Keep) { value = failOperation }, (evt) =>
                {
                    if (Equals(failOperation, evt.newValue))
                        return;
                    registerUndo("Change Stencil Fail Operation");
                    failOperation = (StencilOp)evt.newValue;
                    onChange();
                });

                context.AddProperty("Depth Fail", new EnumField(StencilOp.Keep) { value = zFailOperation }, (evt) =>
                {
                    if (Equals(zFailOperation, evt.newValue))
                        return;
                    registerUndo("Change Stencil Z Fail Operation");
                    zFailOperation = (StencilOp)evt.newValue;
                    onChange();
                });
                context.globalIndentLevel--;
            }
            ...
            context.AddProperty("Additional Motion Vectors", "Specifies how motion vectors for local Shader Graph position modifications are handled (on top of camera, transform and skeletal).", 0, new EnumField(AdditionalMotionVectorMode.None) { value = additionalMotionVectorMode }, (evt) =>
            {
        ...
        void ClearUnusedData()
        {
            ...
            {
                var data = m_Datas[i];
                if (ReferenceEquals(data, null) || ReferenceEquals(data.value, null))
                    continue;
        ...
        public void SetDataOnSubTarget<T>(SubTarget subTarget) where T : JsonObject
        {
            ...
            {
                if (x != null && x.GetType().Equals(typeof(T)))
                {
                    data = x as T;
                    continue;
                }
        ...
        public bool TryUpgradeFromMasterNode(IMasterNode1 masterNode, out Dictionary<BlockFieldDescriptor, int> blockMap)
        {
            ...
            SubTarget preferSubTarget = null;
            switch (masterNode)
            {
                case PBRMasterNode1 pbrMasterNode:
                    ...
                    preferSubTarget = m_SubTargets.FirstOrDefault(x => x.displayName == "Lit");
                    break;
                case UnlitMasterNode1 unlitMasterNode:
                    ...
                    preferSubTarget = m_SubTargets.FirstOrDefault(x => x.displayName == "Unlit");
                    break;
                case SpriteLitMasterNode1 spriteLitMasterNode:
                    ...
                    preferSubTarget = m_SubTargets.FirstOrDefault(x => x.displayName == "Sprite Lit");
                    break;
                case SpriteUnlitMasterNode1 spriteUnlitMasterNode:
                    ...
                    preferSubTarget = m_SubTargets.FirstOrDefault(x => x.displayName == "Sprite Unlit");
                    break;
            }

            {
                // We choose a prefer legacy targets for different MasterNode.
                if (preferSubTarget != null
                    && preferSubTarget is ILegacyTarget legacySubTarget)
                {
                    if (legacySubTarget.TryUpgradeFromMasterNode(masterNode, out blockMap))
                    {
                        m_ActiveSubTarget = preferSubTarget;
                        return true;
                    }
                }
            }
        ...
        public override bool SupportsGDRP()
        {
            if (GraphicsSettings.currentRenderPipeline is UniversalRenderPipelineAsset == false)
                return false;
            if (uncompatibleGDRP != 0)
                return false;
            if (m_CompatibleVGSubTarget.Contains(m_ActiveSubTarget.value.GetType()))
            {
                var subTarget = m_ActiveSubTarget.value as IVGSubTarget;
                return subTarget?.SupportsGDRP() ?? false;
            }
            return false;
        }
    ...
    static class CorePasses
    {
        ...
        public static PassDescriptor DepthOnly(UniversalTarget target, UniversalSubTarget.StencilStatus stencilStatus = null)
        {
            ...
            if (stencilStatus != null)
                UniversalSubTarget.AddStencilStatusControl(ref result, stencilStatus);
        ...
        // used by lit/unlit subtargets
        public static PassDescriptor DepthNormal(UniversalTarget target, UniversalSubTarget.StencilStatus stencilStatus = null)
        {
            var result = new PassDescriptor()
            {
                // Definition
        ...
        // used by lit/unlit subtargets
        public static PassDescriptor DepthNormalOnly(UniversalTarget target, UniversalSubTarget.StencilStatus stencilStatus = null)
        {
            var result = new PassDescriptor()
            {
                // Definition
        ...
        public static PassDescriptor MotionVectors(UniversalTarget target, UniversalSubTarget.StencilStatus stencilStatus = null)
        {
            var result = new PassDescriptor()
            {
                // Definition
            ...
            if (target.additionalMotionVectorMode == AdditionalMotionVectorMode.TimeBasedOffset)
    ...
    static class CoreRenderStates
    {
        public static class Uniforms
        {
            ...
            public static readonly string zClip = "[" + Property.ZClip + "]";
        }
        ...
        public static readonly RenderStateCollection GBufferSplat = new RenderStateCollection
        {
        ...
        public static readonly RenderStateCollection VBufferAlphaClip = new RenderStateCollection
        {
            { RenderState.ZTest(ZTest.LEqual) },
            ...
            {
                WriteMask = "[" + Property.StencilWriteMaskGBufferURP + "]",
                Ref = "[" + Property.StencilRefGBufferURP + "]",
                Comp = "Always",
                Pass = "Replace"
            })}
        };

        public static readonly RenderStateCollection AlphaMeshSelection = new RenderStateCollection
        {
            { RenderState.Cull(Cull.Off) },
            { RenderState.ZTest("Off") },
        };

        public static readonly RenderStateCollection VGShadowMapAlphaClip = new RenderStateCollection
        {
    ...
    #region FeatureName
    static class FeatureName
    {
        public static readonly string ClearCoat = "Clear Coat";
        public static readonly string ThinFilm = "Thin Film";
        public static readonly string DiffractionGratings = "Diffraction Gratings";
        public static readonly string Anisotropy = "Anisotropy";
        public static readonly string CustomIndirectSpecular = "Custom Indirect Specular";
        public static readonly string CustomIndirectDiffuse = "Custom Indirect Diffuse";

        // Just for better readability
        public static readonly Dictionary<string, int> featureDic = new Dictionary<string, int>
        {
            { ClearCoat,   0},
            { ThinFilm,    1},
            { DiffractionGratings, 2},
            { Anisotropy,  3},
            { CustomIndirectSpecular, 4},
            { CustomIndirectDiffuse,  5}
        };
        public static int GetFeatureID(string name)
        {
            return featureDic[name];
        }
    }
    #endregion
    #region PassName
    static class PassName
    {
        public static readonly string forwardPass = "Forward";
        public static readonly string shadowCasterPass = "Shadow Caster";
        public static readonly string depthOnlyPass = "Depth Only";
        public static readonly string depthNormalPass = "Depth Normals";
        public static readonly string gBufferPass = "GBuffer";
        public static readonly string gBufferSplatPass = "GBufferSplat";
        public static readonly string metaPass = "Meta";
        public static readonly string sceneSelectionPass = "Scene Selection";
        public static readonly string scenePickingPass = "Scene Picking";
        public static readonly string _2DPass = "Universal 2D";

        // Just for better readability
        public static readonly Dictionary<string, int> passDic = new Dictionary<string, int>
        {
            { forwardPass,         0},
            { shadowCasterPass,    1},
            { depthOnlyPass,       2},
            ...
            { gBufferSplatPass,    5},
            { metaPass,            6},
            { sceneSelectionPass,  7},
            { scenePickingPass,    8},
            { _2DPass,             9},
        };
        public static int GetPassID(string name)
        {
            return passDic[name];
        }
    }
    ...
    #region KeywordName
    static class KeywordName
    {
        public static readonly string screenSpaceAmbientOcclusion = "Screen Space Ambient Occlusion";
        public static readonly string staticLightmap = "Static Lightmap";
        public static readonly string dynamicLightmap = "Dynamic Lightmap";
        public static readonly string directionalLightmapCombined = "Directional Lightmap Combined";
        public static readonly string mainLightShadows = "Main Light Shadows";
        public static readonly string additionalLights = "Additional Lights";
        public static readonly string additionalLightShadows = "Additional Light Shadows";
        public static readonly string reflectionProbeBlending = "Reflection Probe Blending";
        public static readonly string reflectionProbeBoxProjection = "Reflection Probe Box Projection";
        public static readonly string shadowsSoft = "Shadows Soft";
        public static readonly string lightmapShadowMixing = "Lightmap Shadow Mixing";
        public static readonly string shadowsShadowmask = "Shadows Shadowmask";
        public static readonly string mixedLightingSubtractive = "Mixed Lighting Subtractive";
        public static readonly string dBuffer = "DBuffer";
        public static readonly string lightLayers = "Light Layers";
        public static readonly string lightCookies = "Light Cookies";
        public static readonly string evaluateSH = "Evaluate SH";
        public static readonly string sampleGI = "Sample GI";



        // Just for better readability
        public class KeywordDescripID
        {
            public int ID;
            public KeywordDescriptor desc;
        }
        public static readonly Dictionary<string, KeywordDescripID> keywordDic = new Dictionary<string, KeywordDescripID>
        {
            { screenSpaceAmbientOcclusion,         new KeywordDescripID(){ID = 0, desc = CoreKeywordDescriptors.ScreenSpaceAmbientOcclusion}},
            { staticLightmap,                      new KeywordDescripID(){ID = 1, desc = CoreKeywordDescriptors.StaticLightmap}},
            { dynamicLightmap,                     new KeywordDescripID(){ID = 2, desc = CoreKeywordDescriptors.DynamicLightmap}},
            { directionalLightmapCombined,         new KeywordDescripID(){ID = 3, desc = CoreKeywordDescriptors.DirectionalLightmapCombined}},
            { mainLightShadows,                    new KeywordDescripID(){ID = 4, desc = CoreKeywordDescriptors.MainLightShadows}},
            { additionalLights,                    new KeywordDescripID(){ID = 5, desc = CoreKeywordDescriptors.AdditionalLights}},
            { additionalLightShadows,              new KeywordDescripID(){ID = 6, desc = CoreKeywordDescriptors.AdditionalLightShadows}},
            { reflectionProbeBlending,             new KeywordDescripID(){ID = 7, desc = CoreKeywordDescriptors.ReflectionProbeBlending}},
            { reflectionProbeBoxProjection,        new KeywordDescripID(){ID = 8, desc = CoreKeywordDescriptors.ReflectionProbeBoxProjection}},
            { shadowsSoft,                         new KeywordDescripID(){ID = 9, desc = CoreKeywordDescriptors.ShadowsSoft}},
            { lightmapShadowMixing,                new KeywordDescripID(){ID = 10, desc = CoreKeywordDescriptors.LightmapShadowMixing}},
            { shadowsShadowmask,                   new KeywordDescripID(){ID = 11, desc = CoreKeywordDescriptors.ShadowsShadowmask}},
            { mixedLightingSubtractive,            new KeywordDescripID(){ID = 12, desc = CoreKeywordDescriptors.MixedLightingSubtractive}},
            { dBuffer,                             new KeywordDescripID(){ID = 13, desc = CoreKeywordDescriptors.DBuffer}},
            { lightLayers,                         new KeywordDescripID(){ID = 14, desc = CoreKeywordDescriptors.LightLayers}},
            { lightCookies,                        new KeywordDescripID(){ID = 15, desc = CoreKeywordDescriptors.LightCookies}},
            { evaluateSH,                          new KeywordDescripID(){ID = 16, desc = CoreKeywordDescriptors.EvaluateSh}},
            { sampleGI,                            new KeywordDescripID(){ID = 17, desc = CoreKeywordDescriptors.SampleGI}},
        };
        public static int GetKeywordID(string name)
        {
            return keywordDic[name].ID;
        }
        public static KeywordDescriptor GetDesc(string name)
        {
            return keywordDic[name].desc;
        }
    }
    #endregion
    ...
    static class CorePragmas
    {
        ...
        public static readonly PragmaCollection GBufferSplat = new PragmaCollection
        {
            { Pragma.Target(ShaderModel.Target45) },
            { Pragma.OnlyRenderers(new [] { Platform.D3D11, Platform.Vulkan, Platform.Metal }) },
            { Pragma.MultiCompileInstancing },
            { Pragma.MultiCompileFog },
            { Pragma.InstancingOptions(InstancingOptions.RenderingLayer) },
            { Pragma.InstancingOptions(InstancingOptions.Procedural) },
            { Pragma.Vertex("vert") },
            { Pragma.Fragment("frag") },
        };

        public static readonly PragmaCollection AlphaClip = new PragmaCollection
        {
            { Pragma.Target(ShaderModel.Target45) },
            { Pragma.OnlyRenderers(new [] { Platform.D3D11, Platform.Vulkan, Platform.Metal }) },
            { Pragma.MultiCompileInstancing },
    ...
    static class CoreIncludes
    {
        ...
        const string kGBufferSplatProperties = "Packages/com.unity.render-pipelines.universal/Shaders/Utils/GBufferSplatProperties.hlsl";
        const string kReflectionProbeProperties = "Packages/com.unity.render-pipelines.universal/Shaders/Utils/ReflectionProbeProperties.hlsl";
        ...
        public static readonly IncludeCollection GDRPPregraph = new IncludeCollection
        {
            { kColor, IncludeLocation.Pregraph },
            { kTexture, IncludeLocation.Pregraph },
            { kCore, IncludeLocation.Pregraph },
            { kGBufferSplatProperties, IncludeLocation.Pregraph },  // need to include before Lighting.
            { kReflectionProbeProperties, IncludeLocation.Pregraph },
            { kLighting, IncludeLocation.Pregraph },
            { kInput, IncludeLocation.Pregraph },
    ...
    {
        // VG Shadow keyword
        public static readonly KeywordDescriptor MultiView = new KeywordDescriptor()
        {
            displayName = "Multi View",
            referenceName = "MULTI_VIEW",
        ...
        public static readonly KeywordDescriptor ClusteredLighting = new KeywordDescriptor()
        {
            displayName = "Clustered Lighting",
            referenceName = "_CLUSTERED_LIGHTING",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.MultiCompile,
            scope = KeywordScope.Global,
        };
        ...
        public static readonly KeywordDescriptor ThinFilm = new KeywordDescriptor()
        {
            displayName = "Think Film",
            referenceName = "_THIN_FILM",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor CustomIndirectSpecular = new KeywordDescriptor()
        {
            displayName = "Custom Indirect Specular",
            referenceName = "_CUSTOM_INDIRECT_SPECULAR",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor CustomIndirectDiffuse = new KeywordDescriptor()
        {
            displayName = "Custom Indirect Diffuse",
            referenceName = "_CUSTOM_INDIRECT_DIFFUSE",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor DiffractionGratings = new KeywordDescriptor()
        {
            displayName = "Diffraction Gratings",
            referenceName = "_DIFFRACTION_GRATINGS",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor DiffuseModel_Lambert = new KeywordDescriptor()
        {
            displayName = "Diffuse Model Lambertian",
            referenceName = "_DIFFUSE_MODEL_LAMBERT",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor DiffuseModel_Disney = new KeywordDescriptor()
        {
            displayName = "Diffuse Model Disney",
            referenceName = "_DIFFUSE_MODEL_DISNEY",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor SpecularModel_ApproxCookTorrance = new KeywordDescriptor()
        {
            displayName = "Specular Model Approx CookTorrance",
            referenceName = "_SPECULAR_MODEL_APPROX_COOKTORRANCE",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor SpecularModel_BlinnPhong = new KeywordDescriptor()
        {
            displayName = "Specular Model Blinn Phong",
            referenceName = "_SPECULAR_MODEL_BLINNPHONG",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor SpecularModel_Aniso = new KeywordDescriptor()
        {
            displayName = "Specular Model Aniso",
            referenceName = "_SPECULAR_MODEL_ANISO",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.ShaderFeature,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        };
        public static readonly KeywordDescriptor SmoothnessInNormalPass_Off = new KeywordDescriptor()
        {
            displayName = "Smoothness In Normal Pass Off",
            referenceName = "_SMOOTHNESS_IN_NORMALPASS_OFF",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.Predefined,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        ...
        public static readonly KeywordDescriptor UsePreIntegratedFDG = new KeywordDescriptor()
        {
            displayName = "Use PreIntegrated FDG",
            referenceName = "_USE_PREINTEGRATED_FDG",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.Predefined,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        ...
        public static readonly KeywordDescriptor ScalableLit = new KeywordDescriptor()
        {
            displayName = "ScalableLit",
            referenceName = "_SCALABLE_LIT",
            type = KeywordType.Boolean,
            definition = KeywordDefinition.Predefined,
            scope = KeywordScope.Local,
            stages = KeywordShaderStage.Fragment
        ...
        public static readonly KeywordDescriptor UseLightmapSingle = new KeywordDescriptor()
        {
            displayName = "Use Lightmap single",
            referenceName = ShaderKeywordStrings.USE_LIGHTMAP_SINGLE,
            type = KeywordType.Boolean,
            definition = KeywordDefinition.MultiCompile,
            scope = KeywordScope.Global
        };

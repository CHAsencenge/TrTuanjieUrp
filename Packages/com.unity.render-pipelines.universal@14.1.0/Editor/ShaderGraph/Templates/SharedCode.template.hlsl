SurfaceDescriptionInputs BuildSurfaceDescriptionInputs(Varyings input UNITY_GDRP_MATRIX_M_PARAMETER UNITY_GDRP_MATRIX_M_I_PARAMETER UNITY_GDRP_ATTRIBUTE_PARAMETER)
{
    ...
    $SurfaceDescriptionInputs.WorldSpaceBiTangent:                      float oddNegative = determinant(local2WorldMatrix) > 0.0f ? 1.0f : -1.0f;
    $SurfaceDescriptionInputs.WorldSpaceBiTangent:                      float crossSign = (input.tangentWS.w > 0.0 ? 1.0 : -1.0) * oddNegative;
    #else
    ...
    $SurfaceDescriptionInputs.ObjectSpaceTangent:                       output.ObjectSpaceTangent = TransformWorldToObjectDir(output.WorldSpaceTangent UNITY_GDRP_MATRIX_M_I_ARGUMENT);
    ...
    $SurfaceDescriptionInputs.ObjectSpaceBiTangent:                     output.ObjectSpaceBiTangent = TransformWorldToObjectDir(output.WorldSpaceBiTangent UNITY_GDRP_MATRIX_M_I_ARGUMENT);
    ...
    $SurfaceDescriptionInputs.ObjectSpaceViewDirection:                 output.ObjectSpaceViewDirection = TransformWorldToObjectDir(output.WorldSpaceViewDirection UNITY_GDRP_MATRIX_M_I_ARGUMENT);
    ...
    $SurfaceDescriptionInputs.ObjectSpacePosition:                      output.ObjectSpacePosition = TransformWorldToObject(input.positionWS UNITY_GDRP_MATRIX_M_I_ARGUMENT);
    ...
    $SurfaceDescriptionInputs.ObjectSpacePositionPredisplacement:       output.ObjectSpacePositionPredisplacement = TransformWorldToObject(input.positionWS UNITY_GDRP_MATRIX_M_I_ARGUMENT);
...
#ifdef UNITY_GPU_DRIVEN_PIPELINE
    $SurfaceDescriptionInputs.ddxddyUV0:                                output.ddxddyUV0 = float4(attribute.texCoordsDDX[0], attribute.texCoordsDDY[0]);
    $SurfaceDescriptionInputs.ddxddyUV1:                                output.ddxddyUV1 = float4(attribute.texCoordsDDX[1], attribute.texCoordsDDY[1]);
    $SurfaceDescriptionInputs.ddxddyUV2:                                output.ddxddyUV2 = float4(attribute.texCoordsDDX[2], attribute.texCoordsDDY[2]);
    $SurfaceDescriptionInputs.ddxddyUV3:                                output.ddxddyUV3 = float4(attribute.texCoordsDDX[3], attribute.texCoordsDDY[3]);
#endif
        return output;
}

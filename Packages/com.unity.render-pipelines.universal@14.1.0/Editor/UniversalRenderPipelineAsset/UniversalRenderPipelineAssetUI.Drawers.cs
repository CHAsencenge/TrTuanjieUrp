...
using UnityEditor.TuanjieFeatures.Analytics;
using UnityEditor.TuanjieGRD.Analytics;
using System.Linq;

namespace UnityEditor.Rendering.Universal
{
    using static UnityEngine.Rendering.Universal.UniversalRenderPipelineAsset;
    ...
    internal partial class UniversalRenderPipelineAssetUI
    {
        ...
        static void DrawRendering(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor)
        {
            if (ownerEditor is UniversalRenderPipelineAssetEditor urpAssetEditor)
            {
                ...
                EditorGUILayout.PropertyField(serialized.gpuResidentDrawerStat, Styles.gpuResidentDrawerStat);

                if ((GPUResidentDrawerStat)serialized.gpuResidentDrawerStat.intValue != GPUResidentDrawerStat.Disabled)
                {
                    ++EditorGUI.indentLevel;

                    EditorGUILayout.LabelField(Styles.grdInstancePropertiesSettingsText);
                    ++EditorGUI.indentLevel;
                    using (new EditorGUI.DisabledScope(true))
                    {
                        EditorGUILayout.PropertyField(serialized.grdEnableColletTransform, Styles.grdEnableCollectTransform);
                    }
                    EditorGUILayout.PropertyField(serialized.grdEnableColletLightMap, Styles.grdEnableCollectLightmap);
                    EditorGUILayout.PropertyField(serialized.grdEnableColletLightProbe, Styles.grdEnableCollectLightProbe);
                    --EditorGUI.indentLevel;
                    EditorGUILayout.PropertyField(serialized.grdUseTextureStoreData, Styles.grdUseTextureStoreData);

                    int grdUseTextureStoreData = serialized.grdUseTextureStoreData.enumValueIndex;
                    if (grdUseTextureStoreData != (int)GRDDataStoreType.UseTexture)
                    {
                            ...
                            {
                                bool hasGLES = graphicsAPIs.Contains(GraphicsDeviceType.OpenGLES3) || 
                                                graphicsAPIs.Contains(GraphicsDeviceType.OpenGLES2) || 
                                                graphicsAPIs.Contains(GraphicsDeviceType.OpenGLCore);
                                if (hasGLES)
                                {
                                    EditorGUILayout.HelpBox(Styles.grdUseSSBOPlatformGLESMessage.text, MessageType.Warning, true);
                                }
                            }
                    }

                    int staticBatching, dynamicBatching;
                    PlayerSettings.GetUseBatchingModeForPlatform(EditorUserBuildSettings.activeBuildTarget, out staticBatching, out dynamicBatching);

#if UNITY_GPU_DRIVEN_PIPELINE
                    if(GraphicsSettings.enableGDRP)
                        EditorGUILayout.HelpBox(Styles.grdGDRPEnabledMessage.text, MessageType.Warning, true);
#endif

                    if (EditorGraphicsSettings.batchRendererGroupStrippingMode != BrgStrippingMode.KeepAll)
                        EditorGUILayout.HelpBox(Styles.brgStrippingModeIncorrectMessage.text, MessageType.Warning, true);
                    if (!IsGRDRenderingModeSupport(serialized.asset))
                        EditorGUILayout.HelpBox(Styles.grdRenderingModeIncorrectMessage.text, MessageType.Warning, true);


                    if (staticBatching > 0)
                        EditorGUILayout.HelpBox(Styles.enablestaticBatchingMessage.text, MessageType.Info, true);
                ...
                {
                    var newValue = (GPUResidentDrawerStat)serialized.gpuResidentDrawerStat.intValue;
                    TuanjieGRDAnalytics.SendEnableGRDEvent(
                        "Set Gpu Resident Drawer Status to  " + newValue.ToString(), serialized.grdEnableColletLightMap.boolValue, serialized.grdEnableColletLightProbe.boolValue);
                }

                // TODO: hide brg storage variant strip for 1.6.0, change in later version.
                EditorGUILayout.PropertyField(serialized.brgShaderVariantStorageStripType, Styles.brgShaderVariantStorageStripType);

                if ((GPUResidentDrawerStat)serialized.gpuResidentDrawerStat.intValue != GPUResidentDrawerStat.Disabled)
                {

                    int grdUseTextureStoreData = serialized.grdUseTextureStoreData.enumValueIndex;
                    int brgStripTypeIndex = serialized.brgShaderVariantStorageStripType.enumValueIndex;
                    if (grdUseTextureStoreData == (int)GRDDataStoreType.UseTexture && (brgStripTypeIndex == (int)BRGShaderVariantStorageStripType.KeepSSBOOrUniform))
                    {
                        EditorGUILayout.HelpBox(Styles.grdForBrgBufferStripModeTextureIncorrectMessage.text, MessageType.Warning, true);
                    }
                    if (grdUseTextureStoreData == (int)GRDDataStoreType.UseSSBO && (brgStripTypeIndex == (int)BRGShaderVariantStorageStripType.KeepTexture))
                    {
                        EditorGUILayout.HelpBox(Styles.grdForBrgBufferStripModeSSBOIncorrectMessage.text, MessageType.Warning, true);
                    }
                }
            }
        }
        ...
        static void DrawRenderingAdditional(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                var newValue = serialized.srpBatcher.boolValue;
                TuanjieGRDAnalytics.SendSetSRPBatcherEvent("Set SRP Batcher", newValue ? "enabled" : "disabled");
            }
        ...
        static void DrawQuality(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                // When the upscaling filter changes, we reset the advanced upscaling filter list.
                if (serialized.upscalingFilter.enumValueIndex == (int)UpscalingFilterSelection.TJSR)
                {
                    Universal.Analytics.TJSRAnalytics.SendTJSREvent("UseTJSR", serialized.TJSRSettings.FindPropertyRelative("tjsrQuality").enumValueIndex, serialized.TJSRSettings.FindPropertyRelative("m_tjsrContrastAdaptiveSharpening").floatValue);
                }
            }
            ...
            if (serialized.asset.upscalingFilter == UpscalingFilterSelection.TJSR)
            {
                ++EditorGUI.indentLevel;
                EditorGUILayout.PropertyField(serialized.TJSRSettings.FindPropertyRelative("tjsrQuality"), Styles.TjsrQualityText);
                if (serialized.TJSRSettings.FindPropertyRelative("tjsrQuality").enumValueIndex == (int)AntialiasingQuality.High)
                {
                    EditorGUILayout.PropertyField(serialized.TJSRSettings.FindPropertyRelative("m_tjsrContrastAdaptiveSharpening"), Styles.TjsrContrastAdaptiveSharpeningText);
                }
                EditorGUILayout.HelpBox(Styles.TjsrEditorInfo, MessageType.Info);
                --EditorGUI.indentLevel;
            }
            DrawUniversalUpscalerModeList(serialized, ownerEditor);
        ...
        static void DrawUniversalUpscalerModeList(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor)
        {
            if (ownerEditor as UniversalRenderPipelineAssetEditor != null)
            {
                var urpAssetEditor = (UniversalRenderPipelineAssetEditor)ownerEditor;
                var reorderableList = urpAssetEditor.upscalingFilterDataList;
                if (reorderableList == null)
                {
                    reorderableList= new ReorderableList(serialized.serializedObject, serialized.advancedUpscalingFilterList, false, true, true, true)
                    {
                        drawHeaderCallback = rect => EditorGUI.LabelField(new Rect(rect.x - 15f, rect.y, rect.width - 15f, rect.height), "Advanced Upscaling Filter"),
                        drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                        {
                            rect.y += 2;
                            var element = serialized.advancedUpscalingFilterList.GetArrayElementAtIndex(index);
                            EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),
                                element.enumDisplayNames[element.enumValueIndex], EditorStyles.label);
                        },
                        onAddDropdownCallback = (rect, list) =>
                        {
                            int upscalerEnabledMask = GetUpscalerEnabledMask(serialized.advancedUpscalingFilterList);
                            int upscalerRemainingMask = 0;
                            var upscalerArray = Enum.GetValues(typeof(AdvancedUpscalingFilterOption));
                            for (int i = 0; i < upscalerArray.Length; i++)
                            {
                                upscalerRemainingMask |= 1 << (int)upscalerArray.GetValue(i);
                            }
                            upscalerRemainingMask ^= upscalerEnabledMask;
                            // Avoid the None option to be shown in the list. 
                            upscalerRemainingMask &= ~(1 << (int)AdvancedUpscalingFilterOption.None);
                            
                            int availableScalersCount = upscalerArray.Length - serialized.advancedUpscalingFilterList.arraySize - 1;
                            var possible = new AdvancedUpscalingFilterOption[availableScalersCount];
                            var names = new GUIContent[availableScalersCount];
                            var enabled = new bool[availableScalersCount];
            
                            for (int nextItem = 0; upscalerRemainingMask != 0;)
                            {
                                int upscaler = 0, tempUpscalerRemainingMask = upscalerRemainingMask;
                                while (tempUpscalerRemainingMask > 0 && tempUpscalerRemainingMask % 2 == 0)
                                {
                                    tempUpscalerRemainingMask >>= 1;
                                    upscaler++;
                                }

                                enabled[nextItem] = (upscalerEnabledMask & (1 << upscaler)) == 0;
                                possible[nextItem] = (AdvancedUpscalingFilterOption)upscaler;
                                names[nextItem] = new GUIContent(((AdvancedUpscalingFilterOption)upscaler).ToString());
                                upscalerRemainingMask ^= (1 << upscaler); //turn off the bit
                                nextItem++;
                            }
                            ...
                            EditorUtility.SelectMenuItemFunction value = (userData, options, selected) =>
                            {
                                ...
                                serialized.advancedUpscalingFilterList.InsertArrayElementAtIndex(index);
                                var newElement = serialized.advancedUpscalingFilterList.GetArrayElementAtIndex(index);
                        ...
                        onCanAddCallback = list => list.count < Enum.GetNames(typeof(AdvancedUpscalingFilterOption)).Length - 1,
                    };
                    urpAssetEditor.upscalingFilterDataList = reorderableList;
                }
                EditorGUILayout.Space();
                EditorGUILayout.BeginVertical(new GUIStyle() { margin = new RectOffset((EditorGUI.indentLevel + 1) * 10, 0, 0, 0) });
                reorderableList.DoLayoutList();
                EditorGUILayout.EndVertical();
                
                ++EditorGUI.indentLevel;
                var upscalerEnabledMask = GetUpscalerEnabledMask(serialized.advancedUpscalingFilterList);
                if ((upscalerEnabledMask & (1 << (int)AdvancedUpscalingFilterOption.MetalFX)) != 0)
                {
                    EditorGUILayout.PropertyField(serialized.MetalFXMode, Styles.MetalFXModeText);
                    // check editor preview compatibility
                    if (!MetalFXCoreUtils.IsSupported())
                        EditorGUILayout.HelpBox(Styles.metalFxEditorUnsupportedMessage.text, MessageType.Warning);

                    // check build target compatibility
                    if (EditorUserBuildSettings.activeBuildTarget is not (BuildTarget.iOS or BuildTarget.StandaloneOSX))
                        EditorGUILayout.HelpBox(Styles.metalFxPlayerUnsupportedMessage.text, MessageType.Warning);
                    else if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.iOS)
                    {
                        ...
                        if (string.IsNullOrEmpty(PlayerSettings.iOS.targetOSVersionString) ||
                            ...
                            EditorGUILayout.HelpBox(Styles.metalFxIosMinVersionMessage.text, MessageType.Error);
                    }
                    else
                    {
                        // macOS build target
                        if (string.IsNullOrEmpty(PlayerSettings.macOS.targetOSVersion) ||
                            Version.TryParse(PlayerSettings.macOS.targetOSVersion, out var version) && version < new Version(13, 0))
                        {
                            EditorGUILayout.HelpBox(Styles.metalFxMacosMinVersionMessage.text, MessageType.Error);
                        }
                    }

                    if (serialized.MetalFXMode.GetEnumValue<MetalFXModeSelection>() == MetalFXModeSelection.Temporal)
                    {
                        // show warning when render scale < 0.5
                        if (serialized.renderScale.floatValue < 0.5f)
                            EditorGUILayout.HelpBox(Styles.metalFxTemporalMinScaleMessage.text, MessageType.Warning);
                    }
                }
                --EditorGUI.indentLevel;
            }
        ...
        static int GetUpscalerEnabledMask(SerializedProperty serializedProperty)
        {
            int result = 0;
            for (int i = 0; i < serializedProperty.arraySize; i++)
            {
                result |= 1 << serializedProperty.GetArrayElementAtIndex(i).intValue;
            }
        ...
        static void DrawHDR(SerializedUniversalRenderPipelineAsset serialized, Editor ownerEditor)
        {
            ...
            // if we want to preserve alpha during post-processing, we'd better give user a chance to change hdr precision
            if (serialized.hdr.boolValue && (additionalProperties || serialized.postProcessPreserveAlpha.boolValue))
            {
                EditorGUI.indentLevel++;
...
#if UNITY_GPU_DRIVEN_PIPELINE
            if (GraphicsSettings.enableGDRP)
                EditorGUILayout.PropertyField(serialized.reflectionProbeAtlasResolutionProp, Styles.reflectionProbeAtlasResolutionText);
#endif
            EditorGUI.indentLevel--;
        }
        ...
        {
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(serialized.postProcessPreserveAlpha, Styles.preserveAlpha);
            if (EditorGUI.EndChangeCheck())
            {
                if (serialized.postProcessPreserveAlpha.boolValue)
                {
                    TuanjieFeaturesAnalytics.SendPostProcessPreserveAlphaEvent("enabled", serialized.hdr.boolValue, serialized.hdrColorBufferPrecisionProp.intValue, serialized.colorGradingMode.intValue);
                } else
                {
                    TuanjieFeaturesAnalytics.SendPostProcessPreserveAlphaEvent("disabled", serialized.hdr.boolValue, serialized.hdrColorBufferPrecisionProp.intValue, serialized.colorGradingMode.intValue);
                }
            }

            bool isHdrOn = serialized.hdr.boolValue;
            bool alphaAvailable = !isHdrOn || (HDRColorBufferPrecision)serialized.hdrColorBufferPrecisionProp.intValue == HDRColorBufferPrecision._64Bits;
            if (serialized.postProcessPreserveAlpha.boolValue && !alphaAvailable)
            {
                EditorGUILayout.HelpBox(Styles.alphaNotAvailableWarning, MessageType.Warning);
            }

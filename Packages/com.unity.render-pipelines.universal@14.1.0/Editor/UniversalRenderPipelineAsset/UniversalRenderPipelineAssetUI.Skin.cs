...
namespace UnityEditor.Rendering.Universal
{
    internal partial class UniversalRenderPipelineAssetUI
    {
        internal static class Styles
        {
            ...
            // Quality
            public static string TjsrEditorInfo = "Please enable FrameDebugger or enter Play mode, to make sure the visual fidelity of temporal TJSR take effect.";
            public static GUIContent TjsrQualityText = EditorGUIUtility.TrTextContent("TJSR Quality", "The quality level of Tuanjie Super Resolution.");
            public static GUIContent TjsrContrastAdaptiveSharpeningText = EditorGUIUtility.TrTextContent("TJSR Contrast Adaptive Sharpening", "Enables high quality post sharpening to reduce TJSR blur.");
            public static GUIContent MetalFXModeText = EditorGUIUtility.TrTextContent("MetalFX Mode", "MetalFX upscaling mode on compatible Apple devices.");
            ...
            public static GUIContent metalFxEditorUnsupportedMessage = EditorGUIUtility.TrTextContent("MetalFX editor preview is only available on Mac editor.");
            public static GUIContent metalFxPlayerUnsupportedMessage = EditorGUIUtility.TrTextContent("Current build target does not support MetalFX. MetalFX is only available in iOS and Mac players.");
            public static GUIContent metalFxIosMinVersionMessage = EditorGUIUtility.TrTextContent("MetalFX requires iOS 16.0+. Please configure the target minimum iOS version in player settings.");
            public static GUIContent metalFxMacosMinVersionMessage = EditorGUIUtility.TrTextContent("MetalFX requires macOS 13.0+. Please configure the target minimum macOS version in player settings.");
            public static GUIContent metalFxTemporalMinScaleMessage = EditorGUIUtility.TrTextContent("MetalFX TAAU a minimum render scale of 0.5. Will fall back to MetalFX Spatial for render scale < 0.5.");
            ...
            public static GUIContent reflectionProbeAtlasResolutionText = EditorGUIUtility.TrTextContent("Atlas Resolution", "For VG instances, reflection probe are packed into a single atlas. This setting controls the atlas size.");
            ...
            public static GUIContent preserveAlpha = EditorGUIUtility.TrTextContent("Preserve Alpha", "Preserve alpha channel during post-processing. This will not work if you're using R11G11B10 HDR format.");
            public static string alphaNotAvailableWarning = "Render target doesn't have alpha-channel to preserve.";

            // GPU Resident Drawer
            public static GUIContent gpuResidentDrawerStat = EditorGUIUtility.TrTextContent("GPU Resident Drawer (Experimental)", "Enables draw submission through the GPU Resident Drawer, which can improve CPU performance");
            public static GUIContent grdInstancePropertiesSettingsText = EditorGUIUtility.TrTextContent("Instanced Properties");
            public static GUIContent grdEnableCollectTransform = EditorGUIUtility.TrTextContent("Transform", "Allow GRD to collect the Transform information of the Renderer, including the object2World and world2object matrices.");
            public static GUIContent grdEnableCollectLightmap = EditorGUIUtility.TrTextContent("Lightmap", "Allow GRD to collect the LightmapST of the Renderer.");
            public static GUIContent grdEnableCollectLightProbe = EditorGUIUtility.TrTextContent("Light Probe", "Allow GRD to collect the SHCoefficients of the Renderer.");
            public static GUIContent grdEnableCollectMotionVector = EditorGUIUtility.TrTextContent("Motion Vector", "Allow GRD to collect the Motion Vector of the Renderer.");
            public static GUIContent brgShaderVariantStorageStripType = EditorGUIUtility.TrTextContent("BRG Shader Variant Strip", "Select which BRG shader variants to keep during the stripping process.");
            public static GUIContent brgStrippingModeIncorrectMessage = EditorGUIUtility.TrTextContent("\"BatchRendererGroup Variants\" setting must be \"Keep All\". To fix, modify Graphics settings and set \"BatchRendererGroup Variants\" to \"Keep All\".");
            public static GUIContent enablestaticBatchingMessage = EditorGUIUtility.TrTextContent("Static Batching is not recommended when using GPU draw submission modes, performance may improve if Static Batching is disabled in Player Settings.");
            public static GUIContent grdRenderingModeIncorrectMessage = EditorGUIUtility.TrTextContent("Rendering Path must be set to Forward+ or Forward for correct lighting and reflections. One or more entries in the RendererList are not set to this mode.");
            public static GUIContent grdGDRPEnabledMessage = EditorGUIUtility.TrTextContent("GRD is not supported because GDRP is enabled.");

            public static GUIContent grdForBrgBufferStripModeIncorrectMessage = EditorGUIUtility.TrTextContent("BRG Shader Variant Strip Mode must be \"Keep Texture\" or \"Keep All\". To fix, modify BRG Shader Variant Strip Mode.");
            public static GUIContent grdUseTextureStoreData = EditorGUIUtility.TrTextContent("GRD Data Storage", "Choose Use Texture Or SSBO to Store GRD Data.");
            public static GUIContent grdUseSSBOPlatformGLESMessage = EditorGUIUtility.TrTextContent("The currently used Graphics API does not support SSBO");
            public static GUIContent grdForBrgBufferStripModeSSBOIncorrectMessage = EditorGUIUtility.TrTextContent("BRG Shader Variant Storage option must be \"Keep SSBO or Uniform\" or \"Keep All\".");
            public static GUIContent grdForBrgBufferStripModeTextureIncorrectMessage = EditorGUIUtility.TrTextContent("BRG Shader Variant Storage option must be \"Keep Texture\" or \"Keep All\".");

...
namespace ShaderStrippingAndPrefiltering
{
    ...
    class ShaderScriptableStripperTests
    {
        ...
        [TestCase("Hidden/Universal Render Pipeline/TJSR", false, false)]
        ...
        [TestCase("Hidden/Universal Render Pipeline/TJSR")]
        ...
        [TestCase("Hidden/Universal Render Pipeline/TJSR", false, false, true)]
        ...
        [TestCase("Hidden/Universal Render Pipeline/TJSR", true, true, true)]
        ...
        public void TestStripUnusedFeatures(string shaderName)
        {
            ...
            TestStripUnusedFeatures_ClusteredLighting(shader);
            ...
            TestStripUnusedFeatures_TJSR(shader);
        }
        ...
        public void TestStripUnusedFeatures_ClusteredLighting(Shader shader)
        {
            TestHelper helper;
            
            helper = new TestHelper(shader, ShaderFeatures.None);
            TestHelper.s_PassKeywords = new List<string>() { ShaderKeywordStrings.ForwardPlus };
            helper.IsFalse(helper.stripper.StripUnusedFeatures_ClusteredLighting(ref helper.data));

            helper = new TestHelper(shader, ShaderFeatures.None);
            TestHelper.s_EnabledKeywords = new List<string>() { ShaderKeywordStrings.ForwardPlus };
            TestHelper.s_PassKeywords = new List<string>() { ShaderKeywordStrings.ForwardPlus };
            helper.AreEqual(shader != null, helper.stripper.StripUnusedFeatures_ClusteredLighting(ref helper.data));
        ...
        public void TestStripUnusedFeatures_TJSR(Shader shader)
        {
            bool isCorrectShader = shader == Shader.Find("Hidden/Universal Render Pipeline/TJSR");
            if (!isCorrectShader)
                return;
            TestHelper helper;

            helper = new TestHelper(shader, ShaderFeatures.None);
            helper.AreEqual(isCorrectShader, helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            // TJSR High quality level
            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRReconstruct;
            helper.IsTrue(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRSetup;
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRActive;
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            helper.data.stripDebugDisplayShaders = true;
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRDebugView;
            helper.IsTrue(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            helper.data.stripDebugDisplayShaders = true;
            TestHelper.s_EnabledKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_DEBUGVIEW };
            TestHelper.s_PassKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_DEBUGVIEW };
            helper.IsTrue(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            TestHelper.s_PassKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_INTERMEDIATE_COLOR };
            helper.IsTrue(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            helper = new TestHelper(shader, ShaderFeatures.TJSRHigh);
            TestHelper.s_EnabledKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_HAS_ACTIVE_PASS };
            TestHelper.s_PassKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_HAS_ACTIVE_PASS };
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

            // TJSR Medium quality level
            helper = new TestHelper(shader, ShaderFeatures.TJSRMedium);
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRReconstruct;
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));
            ...
            helper = new TestHelper(shader, ShaderFeatures.TJSRMedium);
            TestHelper.s_EnabledKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_INTERMEDIATE_COLOR };
            TestHelper.s_PassKeywords = new List<string>() { TJSR.ShaderKeywords.TJSR_INTERMEDIATE_COLOR };
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));
            ...
            // TJSR Low quality level
            helper = new TestHelper(shader, ShaderFeatures.TJSRLow);
            helper.data.passName = TJSR.ShaderPassResources.kPassNameTJSRReconstruct;
            helper.IsFalse(helper.stripper.StripUnusedFeatures_TJSR(ref helper.data, ref helper.featureStripTool));

...
#include "Packages/com.tr.render/ShaderLibrary/Debug/Debugging3D.hlsl"
#include "Packages/com.tr.render/ShaderLibrary/GlobalIllumination.hlsl"
#include "Packages/com.tr.render/ShaderLibrary/RealtimeLights.hlsl"
#include "Packages/com.tr.render/ShaderLibrary/AmbientOcclusion.hlsl"
#include "Packages/com.tr.render/ShaderLibrary/DBuffer.hlsl"
...
half BlinnPhongSpecular(float NdotH, half smoothness)
{
    smoothness = exp2(10 * smoothness + 1);
    half normalize = (smoothness + 7) * INV_PI8; // bling-phong energy conerv coeff
    half specular = max(pow(NdotH, smoothness) * normalize, 0.001);
    return specular;
}
...
half3 LightingCustomizable(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, InputData inputData, 
    half clearCoatMask, bool specularHighlightsOff
    ...
    , half slitsDistance, half slitsMask, half3 slitsDirection
#endif
    )
{
    ...
    // We keep it consistent with original URP Lit by multiplying a PI
    half3 radiance = lightColor * NdotL* PI;
    ...
    diffuse = brdfData.diffuse * FabricLambert(brdfData.roughness);
#elif _DIFFUSE_MODEL_LAMBERT
    diffuse = brdfData.diffuse * Lambert();
#elif _DIFFUSE_MODEL_DISNEY
    diffuse = brdfData.diffuse * DisneyDiffuse(NdotV, NdotL, LdotH, brdfData.perceptualRoughness);  
#endif
    ...
    [branch] if (!specularHighlightsOff)
    {
            ...
            specular += F * Vis * D;
        #elif _SPECULAR_MODEL_APPROX_COOKTORRANCE
            specular += brdfData.specular * DirectBRDFSpecular(brdfData, NdotH, LdotH) / PI;
        #elif _SPECULAR_MODEL_BLINNPHONG
            specular += brdfData.specular * BlinnPhongSpecular(NdotH, 1 - brdfData.perceptualRoughness);
        #elif _SPECULAR_MODEL_ANISO
        ...
        // Clear coat evaluates the specular a second timw and has some common terms with the base specular.
        // We rely on the compiler to merge these and compute them only once.
        half brdfCoat = kDielectricSpec.r * DirectBRDFSpecular(brdfDataClearCoat, clearCoatNormal, lightDirWS, viewDirWS);
        ...
        specular = specular * (1.0 - clearCoatMask * coatFresnel) + brdfCoat * clearCoatMask;
        diffuse = diffuse * (1.0 - clearCoatMask * coatFresnel) + brdfCoat * clearCoatMask;
#endif // _CLEARCOAT
    }
#endif // _SPECULARHIGHLIGHTS_OFF

    half3 color = (specular + diffuse) * radiance;

#if _DIFFRACTION_GRATINGS
    half3 diffractionGrating = DiffractionGrating(lightDirWS, viewDirWS, slitsDirection, inputData.normalWS, slitsDistance) * slitsMask;
    return color  + radiance * diffractionGrating * INV_FOUR_PI;
#else
    return color;
#endif
}
...
////////////////////////////////////////////////////////////////////////////////
half4 CustomizableFragmentLighting(InputData inputData, SurfaceData surfaceData)
{
    #if defined(_SPECULARHIGHLIGHTS_OFF)
    bool specularHighlightsOff = true;
    #else
    ...
    float viewAngle = saturate(dot(normalize((float3)inputData.normalWS), normalize((float3)inputData.viewDirectionWS)));
    float topIor = 1.0; // Default is air
    #if (defined(_CLEARCOAT) || defined(_CLEARCOATMAP))
        topIor = lerp(1.0, 1.52f, surfaceData.clearCoatMask);
        // HACK: Use the reflected direction to specify the Fresnel coefficient for pre-convolved envmaps
        if (surfaceData.clearCoatMask != 0.0f) // We must make sure that effect is neutral when coatMask == 0
            viewAngle = sqrt(1.0 + Sq(1.0 / topIor) * (Sq(dot(normalize((float3)inputData.normalWS), normalize((float3)inputData.viewDirectionWS))) - 1.0));
    #endif
    if (surfaceData.thinFilmMask > 0)
    {
        iridescence = saturate(EvalIridescence(topIor, viewAngle, (float)surfaceData.thinFilmThickness, (float3)brdfData.specular));
        brdfData.specular = lerp(brdfData.specular, iridescence, surfaceData.thinFilmMask);
    }
#endif
...
#if defined(_SPECULAR_MODEL_ANISO)
    float3 tangentWS = (TransformTangentToWorld(surfaceData.tangentTS, inputData.tangentToWorld));
    tangentWS = normalize(tangentWS - dot(tangentWS, inputData.normalWS) * inputData.normalWS);
    float3 bitangentWS = cross(inputData.normalWS, tangentWS);
    half3 iblN;
    half iblRoughness = brdfData.perceptualRoughness;
    GetGGXAnisotropicModifiedNormalAndRoughness(bitangentWS, tangentWS, inputData.normalWS, inputData.viewDirectionWS, surfaceData.anisotropy, brdfData.perceptualRoughness, iblN, iblRoughness);
#endif

#if (defined(_CLEARCOAT) || defined(_CLEARCOATMAP)) && defined(_SCALABLE_LIT)
    half3 clearCoatNormal = SafeNormalize(TransformTangentToWorld(surfaceData.clearCoatNormal, inputData.tangentToWorld));
#endif
    LightingData lightingData = CreateLightingData(inputData, surfaceData);
#if defined(_DIFFRACTION_GRATINGS)
    half3 slitsDirection = SafeNormalize(TransformTangentToWorld(surfaceData.slitsDirection, inputData.tangentToWorld));
#endif
#if _GLOBAL_ILLUMINATION_OFF
    lightingData.giColor = 0;
#else
    lightingData.giColor = GlobalIllumination(brdfData, brdfDataClearCoat, inputData, surfaceData, aoFactor.indirectAmbientOcclusion
                                            #if (defined(_CLEARCOAT) || defined(_CLEARCOATMAP)) && defined(_SCALABLE_LIT)
                                                , clearCoatNormal
                                            #endif
                                            #if defined(_SPECULAR_MODEL_ANISO)
                                            , tangentWS, bitangentWS, iblN
                                            #endif
                                            #if defined(_THIN_FILM)
                                            , iridescence
                                            #endif
...
#endif
    {
        lightingData.mainLightColor = LightingCustomizable(brdfData, brdfDataClearCoat,
                                                              mainLight,
                                                              inputData,
                                                              surfaceData.clearCoatMask, specularHighlightsOff
                                                        #if (defined(_CLEARCOAT) || defined(_CLEARCOATMAP)) && defined(_SCALABLE_LIT)
                                                        ...
                                                        #endif
                                                        #if defined(_SPECULAR_MODEL_ANISO)
                                                            , tangentWS, bitangentWS
                                                        #endif
                                                        #if defined(_DIFFRACTION_GRATINGS)
                                                        , surfaceData.slitsDistance, surfaceData.slitsMask, slitsDirection
                                                        #endif
                                                        );
    }
...
#endif
        {
            lightingData.additionalLightsColor += LightingCustomizable(brdfData, brdfDataClearCoat, light,
                                                                          inputData,
                                                                          surfaceData.clearCoatMask, specularHighlightsOff
                                                        #if (defined(_CLEARCOAT) || defined(_CLEARCOATMAP)) && defined(_SCALABLE_LIT)
                                                            , clearCoatNormal

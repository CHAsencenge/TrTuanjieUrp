...
#include "Packages/com.tr.render/ShaderLibrary/Debug/DebuggingCommon.hlsl"
...
#include "Packages/com.tr.render/ShaderLibrary/Shadows.hlsl"
#include "Packages/com.tr.render/ShaderLibrary/SurfaceData.hlsl"
    ...
    else
    {
        ...
        else
        {
            if (UpdateSurfaceAndInputDataForDebug(surfaceData, inputData))
            {
                ...
                #else
                // For debug, maybe we will support GPU Driven LightProbes later, but now it is unsupported.
                float3 positionWS = (float3)0;
                half4 shadowMask;
                // @TODO: Now VG will not access here, but this is included in Lighting.hlsl. So here we should use macro to
                // distinguish different cases, and we just give zero to VG pass. Maybe we will support this debug later.
                #ifndef UNITY_GPU_DRIVEN_PIPELINE
                inputData.bakedGI = SAMPLE_GI(inputData.staticLightmapUV, inputData.vertexSH, inputData.normalWS);
                #else
                float4 ddxddy = (float4)0;
                InstanceSubset instance = (InstanceSubset)0;
                inputData.bakedGI = SAMPLE_GI(inputData.staticLightmapUV, inputData.vertexSH, inputData.normalWS, ddxddy.xy, ddxddy.wz, positionWS, shadowMask, instance);
                #endif
                #endif
            }
        }
